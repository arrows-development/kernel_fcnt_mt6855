...
int EAR_DET_EN = 0; //gpio4
int EAR_DET_IN = 0; //gpio6
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	} else if( 1 == switch_state) {
		gpio_set_value(EAR_DET_EN, 0);
		gpio_set_value(EAR_DET_IN, 0);
	} else if(2 == switch_state) {
		gpio_set_value(EAR_DET_EN, 0);
		gpio_set_value(EAR_DET_IN, 1);
	} else if(3 == switch_state) {
		set_detect_flag(0);
	}

	printk(KERN_ERR "%s rust_debug set_switch_state: %d  EAR_DET_EN =%d   EAR_DET_IN = %d \n",__func__, switch_state,EAR_DET_EN,EAR_DET_IN);
...
static int mt6369_parse_dt(struct mt6369_priv *priv)
{
	...
	dev_info(dev, "rust_debug start read dts \n");
	printk(KERN_ERR " rust_debug start read dts\n");

	EAR_DET_EN = of_get_named_gpio(np,"engpios",0);
	if (gpio_is_valid(EAR_DET_EN)) {
		printk(" EAR_DET_EN is  vaild\n");
		dev_info(dev, "rust_debug EAR_DET_EN is  vaild \n");
	}
	ret = gpio_request(EAR_DET_EN, "EAR_DET_IN");
	if (ret < 0)
		dev_info(dev, "rust_debug EAR_DET_EN requset  failed \n");
	gpio_direction_output(EAR_DET_EN, 1);
	gpio_set_value(EAR_DET_EN, 0);

	EAR_DET_IN = of_get_named_gpio(np,"ingpios",0);
	if (gpio_is_valid(EAR_DET_IN)) {
		printk(" EAR_DET_IN is  vaild\n");
		dev_info(dev, "rust_debug EAR_DET_IN is  vaild \n");
	}
	...
	gpio_set_value(EAR_DET_IN, 1);
	printk(KERN_ERR " rust_debug end read dts EAR_DET_EN = %d, EAR_DET_IN = %d \n",EAR_DET_EN,EAR_DET_IN);
	dev_info(dev, "rust_debug end read dts \n");
