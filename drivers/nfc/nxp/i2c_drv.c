...
int nfc_parse_boot_mode(struct device *dev)
{
...
int nfc_i2c_dev_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct platform_gpio *nfc_gpio = NULL;
        ...
        nfc_boot_mode = nfc_parse_boot_mode(&client->dev);

        pr_info("%s: enter nfc_boot_mode = %d\n", __func__, nfc_boot_mode);

        if (nfc_boot_mode == 8) {
            pr_err("%s: nfc off charging mode, return\n", __func__);
            return 0;
        }
