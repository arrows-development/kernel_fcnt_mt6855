...
#define BATTERY_FULL_DESIGN_CAPACITY 4365000
...
int cycle_flag = 0;
int cycle_count_set = 1;
bool cycle_clear = false;
int cycle_count = 0;

extern bool set_cycle_clear(void);
extern int get_cycles_count(int cycle);
extern int get_battery_age(int age);
extern int get_chg_notify_code(void);
extern bool get_hw_rechg_status(void);
extern bool get_usb_over_temp(void);
...
const char *chg_get_cmd(void)
{
	...
	if (of_chosen) {
		...
		if (!bootargs)
			bm_err("%s: failed to get bootargs\n", __func__);
		else {
...
static int get_batid_by_proinfo(void)
{
        char atm_str[64] = {0};
	char *ptr = NULL, *ptr_e = NULL;
	char keyword[] = "plread.batid=";
	int size = 0;
	struct mtk_gauge *gauge;
	struct power_supply *psy;
	static struct mtk_battery *gm;
	...
	if (ptr != 0) {
		...
		if (!strncmp(atm_str, "First", strlen("First"))) {
			gm->bat_id = 1;
			return 0;
		}
                else if (!strncmp(atm_str, "Second", strlen("Second"))) {
			gm->bat_id = 2;
			return 1;
		}
		else {
...
static int get_bat_health(struct mtk_battery *gm)
{
	struct battery_data *bat_data = &gm->bs_data;
	int notify_code = 0;

	notify_code = get_chg_notify_code();
	if (gm->notify_code == 0) {
		if (notify_code == CHG_VBUS_OV_STATUS)
			bat_data->bat_health = POWER_SUPPLY_HEALTH_OVERVOLTAGE;
		else if (force_get_tbat(gm, true) < 0)
			bat_data->bat_health = POWER_SUPPLY_HEALTH_COLD;
		else if (notify_code == CHG_TYPEC_WD_STATUS)
			bat_data->bat_health = POWER_SUPPLY_HEALTH_WATCHDOG_TIMER_EXPIRE;
		else if (notify_code == CHG_ST_TMO_STATUS)
			bat_data->bat_health = POWER_SUPPLY_HEALTH_SAFETY_TIMER_EXPIRE;
		else if (notify_code == CHG_VBUS_LV_STATUS)
			bat_data->bat_health = POWER_SUPPLY_HEALTH_OVERCURRENT;
		else if (bat_data->bat_batt_vol > BAT_OVER_VOLTAGE4580MV)
			bat_data->bat_health = POWER_SUPPLY_HEALTH_CALIBRATION_REQUIRED;
		else if (notify_code == CHG_USB_OT_STATUS)
			bat_data->bat_health = POWER_SUPPLY_HEALTH_WATCHDOG_TIMER_EXPIRE;
		else if (notify_code == CHG_DEVICE_HOT_STATUS)
			bat_data->bat_health = POWER_SUPPLY_HEALTH_UNSPEC_FAILURE;
		else
			bat_data->bat_health = POWER_SUPPLY_HEALTH_GOOD;
	} else
		bat_data->bat_health = gm->notify_code;

	get_usb_over_temp();

	bm_err("%s:notify_code = %d,bat_health=%d\n", __func__, gm->notify_code, bat_data->bat_health);
	printk("bat id is:%d\n",gm->bat_id);
	return  bat_data->bat_health;
}

int fgauge_get_profile_id(void)
{
	return get_batid_by_proinfo();
}
...
static int get_cycle_count(void) {
	int val;
	int battery_age;
	struct mtk_gauge *gauge;
	struct power_supply *psy;
	...
	cycle_clear = set_cycle_clear();
	if(cycle_clear) {
		gm->is_reset_battery_cycle = true;
		wakeup_fg_algo_cmd(gm,FG_INTR_BAT_CYCLE,0,0);
		wakeup_fg_algo_cmd(gm, FG_INTR_KERNEL_CMD,
				FG_KERNEL_CMD_RESET_AGING_FACTOR, 0);
	}
	cycle_count = val;
	get_cycles_count(val);

	battery_age = gm->algo_qmax * gm->aging_factor / 100;
	if(battery_age > BATTERY_FULL_DESIGN_CAPACITY)
		battery_age = BATTERY_FULL_DESIGN_CAPACITY;
	battery_age = (battery_age *100) / BATTERY_FULL_DESIGN_CAPACITY;
	get_battery_age(battery_age);
	printk("get_cycle_count get battery cycle count %d,cycle_clear %d battery_age=%d!!",val,cycle_clear,battery_age);
	return val;
}
	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		if (IS_ERR_OR_NULL(chg_psy)) {
			...
			ret = power_supply_get_property(chg_psy,
		 	...
		 	bs_data->bat_status = bat_status.intval;
		}
		status = get_hw_rechg_status();
		if (status)
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL:
		val->intval =
			gm->algo_qmax * gm->aging_factor / 100;
		break;
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
		if (check_cap_level(bs_data->bat_capacity) ==
			...
			val->intval = BATTERY_FULL_DESIGN_CAPACITY;
		}
		break;
	...
	case POWER_SUPPLY_PROP_MODEL_NAME:
		if(gm->bat_id == 1)
			val->strval = "SB18E18953_4500mAh_RA07500-1091_FEB18TH2024";
		else if(gm->bat_id == 2)
			val->strval = "SB18E18954_4500mAh_RA07500-1092_JAN23TH2024";
		else
			val->strval = "NULL";
		break;
	default:
	...
	const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CYCLE_COUNT:
		cycle_flag = 1;
		cycle_count_set = val->intval;
		printk("JOJO: u set battery cycle count %d!!",cycle_count_set);
		break;
...
void battery_update(struct mtk_battery *gm)
{
	...
	bat_data->bat_health = get_bat_health(gm);
...
static int fac_soc_get(struct mtk_battery *gm,
	struct mtk_battery_sysfs_field_info *attr,
	int *val)
{
	*val = gm->fac_soc;
...
static int fac_soc_set(struct mtk_battery *gm,
	struct mtk_battery_sysfs_field_info *attr,
	int val)
{
	gm->fac_soc = val;
...
static int bat_id_get(struct mtk_battery *gm,
	struct mtk_battery_sysfs_field_info *attr,
	int *val)
{
	*val = gm->bat_id;
...
static int bat_cycle_count_get(struct mtk_battery *gm,
	struct mtk_battery_sysfs_field_info *attr,
	int *val)
{
	*val = cycle_count_set;
...
static int bat_cycle_count_set(struct mtk_battery *gm,
	struct mtk_battery_sysfs_field_info *attr,
	int val)
{
	cycle_flag = 1;
...
static int fg_v_soc_get(struct mtk_battery *gm,
	struct mtk_battery_sysfs_field_info *attr,
	int *val)
{
	*val = gm->fg_cust_data.v_soc;
	printk("%s %d\n", __func__, *val);
	return 0;
}

static int fg_c_soc_get(struct mtk_battery *gm,
	struct mtk_battery_sysfs_field_info *attr,
	int *val)
{
	*val = gm->fg_cust_data.c_soc;
	printk("%s %d\n", __func__, *val);
	return 0;
}
...
static int notify_code_get(struct mtk_battery *gm,
	struct mtk_battery_sysfs_field_info *attr,
	int *val)
{
	*val = gm->notify_code;
...
static int notify_code_set(struct mtk_battery *gm,
	struct mtk_battery_sysfs_field_info *attr,
	int val)
{
	gm->notify_code = val;
...
static struct mtk_battery_sysfs_field_info battery_sysfs_field_tbl[] = {
	...
	BAT_SYSFS_FIELD_RW(fac_soc, BAT_PROP_FAC_SOC),
	BAT_SYSFS_FIELD_RO(bat_id, BAT_PROP_BAT_ID),
	BAT_SYSFS_FIELD_RW(bat_cycle_count, BAT_PROP_BAT_CYCLE_COUNT),
	BAT_SYSFS_FIELD_RO(fg_c_soc, BAT_PROP_FG_C_SOC),
	BAT_SYSFS_FIELD_RO(fg_v_soc, BAT_PROP_FG_V_SOC),
	BAT_SYSFS_FIELD_RW(notify_code, BAT_PROP_NOTIFY_CODE),
};
	...
	struct notifier_block *nb, unsigned long event, void *v)
{
	...
	if (strcmp(psy->desc->name, "battery") == 0) {
		if (gm != NULL) {
			...
			}else if (gm->cur_bat_temp <= LOW_BATTERY_SHUTDOWN_TEMPERATURE) {
				bm_err(
					"%d battery temperature <= %d,shutdown",
					gm->cur_bat_temp, LOW_BATTERY_SHUTDOWN_TEMPERATURE);
				wake_up_overheat(sdc);
			}
		}
	}
