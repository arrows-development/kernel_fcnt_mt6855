...
int get_vbus_level(void) {
	return vbus_level_mv;
}
EXPORT_SYMBOL(get_vbus_level);
		...
		int mV, int mA)
{
	...
	if (type == MTK_PD_APDO_START) {
		...
		tcpm_ret = tcpm_dpm_pd_request(info->tcpc, mV,
					mA, NULL);
		printk("pd_set_cap  MTK_PD_APDO_END，set 9V1.9A\n");
			     ...
			     struct adapter_auth_data *data)
{
	...
	if (apdo_idx != -1) {
		...
		ret = pd_set_cap(dev, MTK_PD_APDO_START, 9000, 1900);
