...
int charger_dev_set_pd_type(struct charger_device *chg_dev, int type)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->set_pd_type)
		return chg_dev->ops->set_pd_type(chg_dev, type);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_set_pd_type);
