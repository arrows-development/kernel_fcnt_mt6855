...
extern int get_charging_current_limit(void);
...
void mtk_pe45_reset(struct chg_alg_device *alg)
{
	...
	if (pe45->state == PE4_RUN || pe45->state == PE4_INIT ||
		...
		pe4_hal_set_adapter_cap_end(alg, 9000, 1900);
	...
	pe4_err("%s max_vbus:%d,max_ibus:%d,max_charger_ibus:%d\n",__func__, pe45->max_vbus,
		pe45->max_ibus, pe45->max_charger_ibus);
}
	...
	int *ibus_current_setting)
{
	...
	for (i = 0; i < pe45_cap->nr; i++) {
		...
		pe4_err("%s max_mv:%d,max_vbus:%d,max_vbus:%d\n",__func__, pe45_cap->max_mv[i],
			pe45->max_vbus, max_vbus);
		if (*voltage != 0 && *voltage <= max_vbus &&
			*voltage >= pe45_cap->min_mv[i]) {
			...
			pe4_err("%s ibus:%d,vbus:%d,ibus_setting:%d, ta_ibus:%d\n",__func__, ibus,
				vbus, ibus_setting, ta_ibus);
			if (ibus <= max_ibus) {
				idx = 1;
				pe4_err("%s ibus <= max_ibus(%d <= :%d)\n",__func__, ibus, max_ibus);
				break;
			}
		}
		...
		if (pe45_cap->pwr_limit[i] && pe45_cap->pdp > 0) {
			...
			if (max_vbus * pe45->max_charger_ibus >= watt) {
				vbus = max_vbus;
				ibus = watt / vbus;
				ibus_setting = max_ibus;
				ta_ibus = pe45_cap->ma[i];
									...
									int *adapter_ibus)
{
	...
	pe4_err("%s: vbus:%d, ibus:%d, adapter_ibus: %d rcable_index:%d max_vbus:%d input_current_limit1:%d\n",
		__func__, *vbus,*ibus,*adapter_ibus,pe45->rcable_index,pe45->max_vbus,pe45->input_current_limit1);
...
int mtk_pe45_get_init_lower_watt(struct chg_alg_device *alg)
{
	...
	mtk_pe45_get_setting_by_watt(alg, &voltage, &adapter_ibus,
		&actual_current, 17100000, &input_current);
...
//	struct charger_data *pdata;
	...
	pe4_dbg("[pe45_cc]vbus:%d:%d,ibus:%d,cibus:%d,ibat:%d icl:%d:%d,ccl:%d,%d,vbat:%d,maxIbus:%d,mivr:%d,input_current1:%d\n",
		pe45->avbus, vbus,
		ibus,
		compare_ibus,
		ibat,
	...
	if (((chg1_mivr) && !thermal_skip) ||
		...
		pe4_err("%s: chg1_mivr =%d,thermal_skip=%d,compare_ibus=%d\n",
				 __func__, chg1_mivr, thermal_skip, compare_ibus);
		oldavbus = pe45->avbus;
		...
		new_watt = (pe45->avbus + 50) * icl;
			pe4_err("%s:[1] ,pe45->avbus=%d,new_watt=%d\n",
				 __func__, pe45->avbus, new_watt);
		} else if (compare_ibus >= (max_icl - icl_threshold)) {
			pe45->avbus = pe45->avbus + 50;
			new_watt = (pe45->avbus + 50) * ibus;
			pe4_err("%s:[2] ,pe45->avbus=%d,ibus=%d,new_watt=%d\n",
				 __func__, pe45->avbus, ibus, new_watt);
		} else if (compare_ibus <= (max_icl - icl_threshold * 2)) {
			new_watt = pe45->avbus * pe45->ibus - 500000;
			pe4_err("%s:[3] ,pe45->avbus=%d,ibus=%d,new_watt=%d\n",
				 __func__, pe45->avbus,pe45->ibus, new_watt);
			pe45->avbus = pe45->avbus - 50;
			pe4_err("%s:[4] ,pe45->avbus=%d\n", __func__, pe45->avbus);
		}
...
static int pe4_sc_set_charger(struct chg_alg_device *alg)
{
	...
	charging_current_limit = get_charging_current_limit();
	...
	if (charging_current_limit != 0) {
		pe4->charger_current1 = charging_current_limit;
		pe4_err("%s:  charger_current1 = %d\n",__func__,pe4->charger_current1);
	}
