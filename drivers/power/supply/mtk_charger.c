...
#define FULL_CHARGE_SETTING_CAPACITY 85
#define FULL_CHARGE_SETTING_STOP_CHARGING 1
#define FULL_CHARGE_SETTING_RECHARGING 0
#define CHARGER_CURRENT_1A 1000000
#define BATTERY_VOLTAGE_4250MV 4250
#define BATTERY_CURRENT_500MA 500000
#define DCP_MAX_CHARGER_VOLTAGE_6400MV  6400000
#define DCP_MIN_CHARGER_VOLTAGE_4000MV  4000000
#define PD_MIN_CHARGER_VOLTAGE_8400MV  7000000
#define VBUS_LEVEL_9V 9000
#define VBUS_LEVEL_5V 5000
#define BAT_OVER_VOLTAGE4580MV 4580
...
bool bm_flag = true;  //battery maintenance flag, default is 1
bool fv_flag = false;
int battery_voltage = 0;
int battery_voltage_adc = 0;
int battery_current = 0;
int charger_voltage = 0;
bool usb_over_temp = false;
bool bat_low_temp = false;
bool chg_over_voltage = false;
bool chg_low_voltage = false;
bool safety_timeout_flag = false;
bool jeita_stop_charging = false;
bool thermal_notify = false;
bool jeita_chg_flag = false;
struct iio_channel *usb_temp_channel;
extern bool is_led_on(void);
extern int get_vbus_level(void);

bool cycle_clear = false;
int cycle_count = 0;
int battery_age = 0;

bool schedule_chg_flag = false;
bool hw_rechg_status = false;
bool hw_rechg_flag = true;

int get_hw_rechg_status(void)
{
	return hw_rechg_status;
}
EXPORT_SYMBOL(get_hw_rechg_status);

int get_cycles_count(int cycle)
{
	cycle_count = cycle;
	return cycle_count;
}
EXPORT_SYMBOL(get_cycles_count);

int get_battery_age(int age)
{
	...
	if (info->age_flag) {
		chr_err("%s age_flag(%d) is true,use set age:%d\n", __func__, info->age_flag, info->age);
		battery_age = info->age;
		return info->age;
	}
...
EXPORT_SYMBOL(get_battery_age);

bool set_cycle_clear(void)
{
	return cycle_clear;
}
EXPORT_SYMBOL(set_cycle_clear);

int get_chg_notify_code(void)
{
	struct power_supply *chg_psy = NULL;
	struct mtk_charger *info = NULL;
	...
	return info->chg_notify_code;
}
EXPORT_SYMBOL(get_chg_notify_code);

static int get_usb_ntc_temp(int *temp_val)
{
	int ret = 0;
	if(usb_temp_channel != NULL) {
		ret = iio_read_channel_processed(usb_temp_channel, temp_val);
		if (ret < 0)
			printk("get_usb_ntc_temp read fail,ret=%d\n", ret);
		chr_info("%s: usb ntc temp = %d\n", __func__, *temp_val);
	}
	printk("usb ntc temp voltage = %d\n",*temp_val);
	return ret;
}

static int get_typec_temp(void)
{
	int vol, i, value,ret;

	ret = get_usb_ntc_temp(&vol);
	if (vol <= 0)
		return -ENOMEM;

	for (i = 0; i < usb_tab_len; i++)
		if (vol >= usb_table[i].vol)
			break;
	if (i > 0 && i < usb_tab_len) {
		value = usb_table[i].temp;
	} else if (i == 0) {
		value = usb_table[0].temp;
	} else {
		value = usb_table[usb_tab_len - 1].temp;
	}
	printk("get_typec_temp value = %d,i=%d\n",value,i);
	value = value / 1000;
	return value;
}
...
int usb_temp_set_cc(bool usb_temp_flag)
{
	struct power_supply *chg_psy = NULL;
	struct mtk_charger *info = NULL;
	...
	if (usb_temp_flag) {
		...
		chr_err("%s usb_temp_flag:%d\n", __func__, usb_temp_flag);
	}else {
		tcpm_typec_disable_function(info->tcpc_dev, false);
		info->chg_notify_code = CHG_DEFAULT_STATUS;
		chr_err("%s usb_temp_flag:%d\n", __func__, usb_temp_flag);
	}
...
int  get_usb_over_temp(void)
{
	int usb_temp = 0;

	usb_temp = get_typec_temp();

	if (usb_over_temp && (usb_temp <= recover_threshold )) {
		usb_temp_set_cc(false);
		usb_over_temp = false;
		printk("usb temp is low than %d,so start charging",recover_threshold);
	}
	return 0;
}
EXPORT_SYMBOL(get_usb_over_temp);
...
/* sw jeita */
void hq_do_sw_jeita_state_machine(struct mtk_charger *info)
{
	...
	if (info->battery_temp >= info->data.temp_t4_thres) {
		...
		if ((sw_jeita->sm == TEMP_ABOVE_T4)
			...
			if (jeita_stop_charging) {
				charger_dev_enable(info->chg1_dev, true);
				jeita_stop_charging = false;
			}
		}
		...
		if ((sw_jeita->sm == TEMP_T1_TO_T2
			...
			if (sw_jeita->sm == TEMP_BELOW_T0) {
				chr_err("[SW_JEITA] Battery Temperature between %d and %d,up to 0 start charging!!\n",
	...
	ret = charger_dev_get_vbat(info->chg1_dev, &battery_voltage_adc);
	battery_voltage_adc = battery_voltage_adc/1000;
	if ((ret < 0) || (battery_voltage_adc <= 0)) {
		battery_voltage_adc = battery_voltage;
		chr_err("[%s] get vbat from adc fail,use fg vbat:battery_voltage(%d)\n", __func__, battery_voltage);
	}
	chr_err("[%s] battery_voltage_adc:%d\n", __func__, battery_voltage_adc);
	...
	if (sw_jeita->sm == TEMP_ABOVE_T4) {
		...
		sw_jeita->cv = 0;
		sw_jeita->ieoc = BATTERY_CURRENT_500MA;
	}
		...
		if (battery_voltage_adc < BATTERY_VOLTAGE_4250MV){
			sw_jeita->cv = info->data.jeita_temp_t1_to_t2_cv;
		}
		sw_jeita->ieoc = 0;
		if ((battery_voltage_adc >= BATTERY_VOLTAGE_4250MV) && (battery_current <= CHARGER_CURRENT_1A/1000)){
			sw_jeita->cv = 0;
		}
	}
	...
	chr_err("[SW_JEITA]preState:%d newState:%d tmp:%d cv:%d ieoc:%d\n",
		sw_jeita->pre_sm, sw_jeita->sm, info->battery_temp,
		sw_jeita->cv, sw_jeita->ieoc);
}

//wangtao
void do_lcd_quiet_temp_state_machine(struct mtk_charger *info)
{
	struct sw_jeita_data *quiet_jeita;
	int  charger_current = 0;
	quiet_jeita = &info->quiet_jeita;
	quiet_jeita->pre_sm = quiet_jeita->sm;
	quiet_jeita->charging = true;

	charger_current = info->chg_data[CHG1_SETTING].thermal_charging_current_limit;

	if(info->led_status) {
		if(charger_current == 0)
			quiet_jeita->charging = false;
	}

	if(charger_current == 0) {
		info->chg_notify_code = CHG_DEVICE_HOT_STATUS;
		thermal_notify = true;
		chr_err("[do_lcd_quiet_temp_state_machine] device temp high,charger_current = %d,stop charging!!!",charger_current,info->led_status);
	}

	if(thermal_notify && charger_current != 0){
		thermal_notify = false;
		info->chg_notify_code = CHG_DEFAULT_STATUS;
		chr_err("[do_lcd_quiet_temp_state_machine] device temp normal,allow charging!!!");
	}
	chr_err("[do_lcd_quiet_temp_state_machine] charger_current = %d,info->led_status=%d\n",charger_current,info->led_status);
}
...
{
	struct mtk_charger *pinfo = dev->driver_data;
	chr_err("%s: %d\n", __func__, pinfo->charging_enable);
	return sprintf(buf, "%d\n", pinfo->charging_enable);
}
...
static ssize_t usb_suspend_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	struct mtk_charger *pinfo = dev->driver_data;
	chr_err("%s: %d\n", __func__, pinfo->usb_suspend);
	return sprintf(buf, "%d\n", pinfo->usb_suspend);
}

static ssize_t usb_suspend_store(struct device *dev,
				   struct device_attribute *attr,
				   const char *buf, size_t size)
{
	struct mtk_charger *pinfo = dev->driver_data;
...
static DEVICE_ATTR_RW(usb_suspend);

static ssize_t usb_temp_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", get_typec_temp());
}
static DEVICE_ATTR_RO(usb_temp);

static ssize_t schedule_chg_show(struct class *dev,
				  struct class_attribute *attr, char *buf)
{
	struct mtk_charger *pinfo = container_of(dev, struct mtk_charger,
						     oem_batt_class);
	chr_err("%s: %d\n", __func__, pinfo->schedule_chg);
	return sprintf(buf, "%d\n", pinfo->schedule_chg);
}

static ssize_t schedule_chg_store(struct class *dev,
				   ...
				   const char *buf, size_t size)
{
	...
	if (kstrtoint(buf, 10, &temp) == 0) {
		...
		if (temp > 0) {
			...
			if (hw_rechg_status) {
				hw_rechg_flag = true;
				pinfo->rechg_flag = false;
				pinfo->hw_rechg_flag = false;
				hw_rechg_status  = false;
				chr_err("recharging：close direct power supply,start charging\n");
			}
		}
	} else {
		chr_err("%s: format error!\n", __func__);
...
static ssize_t fv_reduction_show(struct class *dev,
				  struct class_attribute *attr, char *buf)
{
	struct mtk_charger *pinfo = container_of(dev, struct mtk_charger,
						     oem_batt_class);
	chr_err("%s: %d\n", __func__, pinfo->fv_reduction);
	return sprintf(buf, "%d\n", pinfo->fv_reduction);
}

static ssize_t fv_reduction_store(struct class *dev,
				   struct class_attribute *attr,
				   const char *buf, size_t size)
{
	struct mtk_charger *pinfo = container_of(dev, struct mtk_charger,
...
static ssize_t cycle_count_show(struct class *dev,
				  struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", cycle_count);
}

static ssize_t cycle_count_store(struct class *dev,
				   struct class_attribute *attr,
				   const char *buf, size_t size)
{
	return 0;
...
static ssize_t cycle_clear_show(struct class *dev,
				  struct class_attribute *attr, char *buf)
{

	chr_err("%s: %d\n", __func__, cycle_clear);
	return sprintf(buf, "%d\n", cycle_clear);
}

static ssize_t cycle_clear_store(struct class *dev,
				   struct class_attribute *attr,
				   const char *buf, size_t size)
{
...
static ssize_t age_show(struct class *dev,
				  struct class_attribute *attr, char *buf)
{
	struct mtk_charger *pinfo = container_of(dev, struct mtk_charger,
						     oem_batt_class);
...
static ssize_t age_store(struct class *dev,
				   struct class_attribute *attr,
				   const char *buf, size_t size)
{
	struct mtk_charger *pinfo = container_of(dev, struct mtk_charger,
	...
	if (kstrtoint(buf, 10, &age) == 0){
		if (age > 0) {
			pinfo->age_flag = true;
			pinfo->age = age;
		} else {
			pinfo->age_flag = false;
		}
	} else {
		chr_err("%s: format error!\n", __func__);
	}
...
static CLASS_ATTR_RW(age);

static ssize_t full_charge_setting_show(struct class *dev,
				  struct class_attribute *attr, char *buf)
{
	struct mtk_charger *pinfo = container_of(dev, struct mtk_charger,
						     oem_batt_class);
	chr_err("%s: %d\n", __func__, pinfo->full_charge_setting);
	return sprintf(buf, "%d\n", pinfo->full_charge_setting);
}

static ssize_t full_charge_setting_store(struct class *dev,
				   struct class_attribute *attr,
				   const char *buf, size_t size)
{
	struct mtk_charger *pinfo = container_of(dev, struct mtk_charger,
						     oem_batt_class);
	signed int full_charge_setting;

	if (kstrtoint(buf, 10, &full_charge_setting) == 0)
		pinfo->full_charge_setting = full_charge_setting;
	else
		chr_err("%s: format error!\n", __func__);
	return size;
}
...
static ssize_t otg_switch_show(struct class *dev,
				  struct class_attribute *attr, char *buf)
{
	struct mtk_charger *pinfo = container_of(dev, struct mtk_charger,
						     oem_batt_class);
	charger_dev_is_otg_enabled(pinfo->chg1_dev, &pinfo->otg_switch);
	chr_err("%s: %d\n", __func__, pinfo->otg_switch);
	return sprintf(buf, "%d\n", pinfo->otg_switch);
}

static ssize_t otg_switch_store(struct class *dev,
				   struct class_attribute *attr,
				   const char *buf, size_t size)
{
	struct mtk_charger *pinfo = container_of(dev, struct mtk_charger,
...
static CLASS_ATTR_RW(otg_switch);

static struct attribute *oem_batt_class_attrs[] = {
	...
	&class_attr_otg_switch.attr,
	NULL,
};
ATTRIBUTE_GROUPS(oem_batt_class);

static ssize_t current_ibus_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	struct mtk_charger *pinfo = dev->driver_data;
	chr_err("%s: %d\n", __func__, get_ibus(pinfo));
	return sprintf(buf, "%d\n", get_ibus(pinfo));
}

static DEVICE_ATTR_RO(current_ibus);
...
static int mtk_chg_en_bm_show(struct seq_file *m, void *data)
{
	bool bm_result = bm_flag;

	chr_err("jojo :[%s]: bm_result=%d\n", __func__,bm_result);
	seq_printf(m, "%d\n", bm_result);

	return 0;
}

static int mtk_chg_en_bm_open(struct inode *node, struct file *file)
{
	return single_open(file, mtk_chg_en_bm_show, PDE_DATA(node));
}

static ssize_t mtk_chg_en_bm_write(struct file *file,
		const char *buffer, size_t count, loff_t *data)
{
	int len = 0, ret = 0;
	char desc[32] = {0};
	unsigned int enable = 0;
	struct mtk_charger *info = PDE_DATA(file_inode(file));
	chr_err("jojo[%s]:mtk_chg_en_hz_write \n", __func__);
	...
	chr_err("bad argument, echo [enable] > en_hz\n");
	return count;
}

static const struct proc_ops mtk_chg_en_bm_fops = {
	.proc_open = mtk_chg_en_bm_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
	.proc_write = mtk_chg_en_bm_write,
};
...
static int mtk_battery_notify_VCharger_check(struct mtk_charger *info)
{
#if defined(BATTERY_NOTIFY_CASE_0001_VCHARGER)
	int vchr = 0;
	int min_charger_voltage = 0;
	int max_charger_voltage = 0;
	int vbus_level = 0;

	vbus_level = get_vbus_level();
	chr_err("vbus_level = %d ,pd_type = %d ,chr_type = %d\n",
			 vbus_level, info->pd_type, info->chr_type);
	if (vbus_level == VBUS_LEVEL_9V) {
		min_charger_voltage = PD_MIN_CHARGER_VOLTAGE_8400MV;
		max_charger_voltage = info->data.max_charger_voltage;
		...
		if (info->pd_type != MTK_PD_CONNECT_PE_READY_SNK_PD30 &&
			...
			chr_err("The charger responds slowly,jump check charger OVP!!!\n");
			return false;
		}
	} else if (vbus_level == VBUS_LEVEL_5V){
		max_charger_voltage = DCP_MAX_CHARGER_VOLTAGE_6400MV;
		min_charger_voltage = DCP_MIN_CHARGER_VOLTAGE_4000MV;
	} else {
		chr_err("vbus_level is %d,jump check charger OVP!!!\n",vbus_level);
		return false;
	}

	vchr = get_vbus(info) * 1000; /* uV */
	if (vchr >= max_charger_voltage){
		chg_over_voltage = true;
		info->chg_notify_code = CHG_VBUS_OV_STATUS;
		chr_err("[BATTERY] charger_vol(%d mV) > %d mV,pd_type:%d,chr_type:%d\n",
			vchr / 1000, max_charger_voltage / 1000, info->pd_type, info->chr_type);
		charger_dev_enable_hz(info->chg1_dev, true);
	}else if (vchr <= min_charger_voltage){
		chg_low_voltage = true;
		info->chg_notify_code = CHG_VBUS_LV_STATUS;
		chr_err("[BATTERY] charger_vol(%d mV) < %d mV,pd_type:%d,chr_type:%d\n",
			vchr / 1000, min_charger_voltage / 1000, info->pd_type, info->chr_type);
		charger_dev_enable_hz(info->chg1_dev, true);
		_mtk_enable_charging(info, false);
	}

	if (chg_over_voltage) {
		if ((vchr <= max_charger_voltage) && (vchr >= min_charger_voltage)) {
			charger_dev_enable_hz(info->chg1_dev, false);
			chg_over_voltage = false;
			info->chg_notify_code = CHG_DEFAULT_STATUS;
			chr_err("[BATTERY] (%d mV) <= vchr(%d) <= %d mV,start charging!\n",
			min_charger_voltage / 1000, vchr / 1000, max_charger_voltage / 1000);
		}
	}
...
static void charger_check_status(struct mtk_charger *info)
{
	...
	info->setting.vbat_mon_en = true;
	info->led_status = is_led_on();
	chr_err("%s, info->led_status = %d\n", __func__,info->led_status);
	...
	if (info->enable_sw_jeita == true) {
		if (jeita_chg_flag) {
			charging = true;
			jeita_chg_flag = false;
			chr_err("charger_check_status: jeita_chg_flag is 1 and first plug,allow charging!");
			goto stop_charging;
		}
		hq_do_sw_jeita_state_machine(info);
		do_lcd_quiet_temp_state_machine(info);
		if (info->sw_jeita.charging == false || info->quiet_jeita.charging == false) {
			charging = false;
			goto stop_charging;
		}
	} else {
	...
	if (schedule_chg_flag && info->schedule_chg) {
		charging = true;
		schedule_chg_flag = false;
		chr_err("charger_check_status: schedule_chg is 1 and first plug,allow charging!");
		goto stop_charging;
	}

	if ((info->full_charge_setting == FULL_CHARGE_SETTING_STOP_CHARGING) && (uisoc >= FULL_CHARGE_SETTING_CAPACITY)) {
		info->charge_setting_flag = true;
		charging = false;
		goto stop_charging;
	}
	else if ((info->full_charge_setting == FULL_CHARGE_SETTING_RECHARGING) && (info->charge_setting_flag == true)) {
		info->charge_setting_flag = false;
		charging = true;
	}
	...
	if (info->safety_timeout){
		if (vbat >= BAT_OVER_VOLTAGE4580MV) {
			charging = false;
			safety_timeout_flag = true;
			info->chg_notify_code = CHG_ST_TMO_STATUS;
			chr_err("charger_check_status: charging timeout and battery voltage(%d) above %d,stop charging!", vbat, BAT_OVER_VOLTAGE4580MV);
		}else {
			charger_dev_enable_safety_timer(info->chg1_dev, false);
			charging = true;
			safety_timeout_flag = false;
			info->safety_timeout = false;
			info->chg_notify_code = CHG_DEFAULT_STATUS;
			charger_dev_enable_safety_timer(info->chg1_dev, true);
			chr_err("charger_check_status: charging timeout but battery voltage(%d) below %d, charging!",vbat, BAT_OVER_VOLTAGE4580MV);
		}
	}
	if (info->vbusov_stat)
		charging = false;
	if (info->sc.disable_charger == true)
		charging = false;
	if (info->hw_rechg_flag && hw_rechg_flag) {
		charger_dev_enable(info->chg1_dev, false);
		hw_rechg_flag = false;
		hw_rechg_status = true;
		chr_err("charger_check_status: hw recharging,stop charging!");
	}
	if ((info->rechg_flag == true) && (uisoc <= 97)){
		hw_rechg_flag = true;
		info->rechg_flag = false;
		info->hw_rechg_flag = false;
		hw_rechg_status  = false;
		charger_dev_enable(info->chg1_dev, true);
		chr_err("charger_check_status: recharging!");
	}
stop_charging:
	...
	chr_err("tmp:%d (jeita:%d sm:%d cv:%d en:%d) (sm:%d) en:%d c:%d s:%d ov:%d sc:%d %d %d saf_cmd:%d bat_mon:%d %d full_charge_setting:%d charge_setting_flag:%d fv_reduction:%d\n",
		temperature, info->enable_sw_jeita, info->sw_jeita.sm,
		info->sw_jeita.cv, info->sw_jeita.charging, thermal->sm,
		charging, info->cmd_discharging, info->safety_timeout,
		info->vbusov_stat, info->sc.disable_charger,
		info->can_charging, charging, info->safety_timer_cmd,
		info->enable_vbat_mon, info->batpro_done, info->full_charge_setting,
		info->charge_setting_flag,info->fv_reduction);
...
static int mtk_charger_plug_out(struct mtk_charger *info)
{
	...
	chr_err("%s\n", __func__);
        if (info->schedule_chg) {
		info->cmd_discharging = false;
		info->schedule_chg = false;
		chr_err("%s charger plug out,set cmd_discharging and schedule_chg to 0\n", __func__);
	}
	...
	if(chg_over_voltage || chg_low_voltage) {
		charger_dev_enable_hz(info->chg1_dev, false);
		chg_over_voltage = false;
		chg_low_voltage = false;
		info->chg_notify_code = CHG_DEFAULT_STATUS;
		printk("plug out set chg_over_voltage or to chg_low_voltage false and disable hz\n");
	}

	if(safety_timeout_flag) {
		safety_timeout_flag = false;
		info->chg_notify_code = CHG_DEFAULT_STATUS;
		printk("plug out set safety_timeout to false\n");
	}

	if(thermal_notify){
		thermal_notify = false;
		info->chg_notify_code = CHG_DEFAULT_STATUS;
		printk("plug out set thermal stop charging false\n");
	}
...
static int charger_routine_thread(void *arg)
{
	...
	while (1) {
		...
		usb_temp = get_typec_temp();
		if(usb_temp >= otp_threshold && !usb_over_temp && (get_vbus(info) > otpv_threshold)) {
			usb_over_temp = true;
			info->chg_notify_code = CHG_USB_OT_STATUS;
			usb_temp_set_cc(usb_over_temp);
			printk("usb temp is over %d,so stop charging",otp_threshold);
		} else if(usb_temp <= recover_threshold && usb_over_temp) {
			usb_over_temp = false;
			info->chg_notify_code = CHG_DEFAULT_STATUS;
			usb_temp_set_cc(usb_over_temp);
			printk("usb temp is low than %d,so start charging",otp_threshold);
		}

		chr_err("Vbat=%d vbats=%d vbus:%d ibus:%d I=%d T=%d uisoc:%d type:%s>%s pd:%d swchg_ibat:%d cv:%d,usb_temp:%d\n",
			...
			info->pd_type, get_ibat(info), chg_cv,usb_temp);

		is_charger_on = mtk_is_charger_on(info);
		battery_voltage = get_battery_voltage(info);
		battery_current = get_battery_current(info);
		charger_voltage = get_vbus(info);
...
static int mtk_charger_setup_files(struct platform_device *pdev)
{
	...
	if (ret)
		goto _out;
	ret = device_create_file(&(pdev->dev), &dev_attr_charging_enable);
	if (ret)
		goto _out;

	ret = device_create_file(&(pdev->dev), &dev_attr_usb_suspend);
	if (ret)
		goto _out;
	...
	ret = device_create_file(&(pdev->dev), &dev_attr_call_state);
	if (ret)
		goto _out;

	ret = device_create_file(&(pdev->dev), &dev_attr_current_ibus);
	if (ret)
		goto _out;
	...
	entry = proc_create_data("en_bm", 0644, battery_dir,
			&mtk_chg_en_bm_fops, info);
	if (!entry) {
		ret = -ENODEV;
		goto fail_procfs;
	}
			...
			const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_AVG:
		info->qns_cv = val->intval;
		break;
	case POWER_SUPPLY_PROP_CURRENT_AVG:
		info->qns_charging_current = val->intval;
		break;
	default:
		return -EINVAL;
	}
			...
			unsigned long evt, void *val)
{
	...
	case MTK_TYPEC_WD_STATUS:
		...
		if (pinfo->water_detected == true) {
			...
			pinfo->notify_code = CHG_DEFAULT_STATUS;
		mtk_chgstat_notify(pinfo);
		break;
	}
	charger_dev_set_pd_type(pinfo->chg1_dev,pinfo->pd_type);
	return NOTIFY_DONE;
}
...
static int __init oem_batt_init(struct mtk_charger *info)
{
	int rc = 0;
	info->oem_batt_class.name = "oem_battery";

	info->oem_batt_class.class_groups = oem_batt_class_groups;

	rc = class_register(&info->oem_batt_class);
	if (rc < 0) {
		chr_err("Failed to create oem_batt_class rc=%d\n", rc);
		return -1;
	}
	return 0;
}
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	usb_temp_channel = iio_channel_get(&pdev->dev, "usb_temp-channel");
	if (IS_ERR(usb_temp_channel)) {
		ret = PTR_ERR(usb_temp_channel);
		pr_err("[%s] iio usb_temp_channel not found %d\n", __func__, ret);
	} else {
		pr_err("[%s] get usb_temp_channel success\n", __func__);
	}
...
{
	struct mtk_charger *bcdev = platform_get_drvdata(dev);
	class_unregister(&bcdev->oem_batt_class);
	return 0;
}
