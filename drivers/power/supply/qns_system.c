 ...
 * Qnovo QNS wrapper implementation. Compatible with kernel 5.10.
...
#define CHARGE_CURRENT_PROP	POWER_SUPPLY_PROP_CURRENT_AVG
#define CHARGE_VOLTAGE_PROP	POWER_SUPPLY_PROP_VOLTAGE_AVG

#define IBATMANAME		"battery"
#define ICHGMANAME		"mtk-master-charger"
...
static int qns_set_ibat(int ibatmA)
{
	...
	if(data.chg_psy == NULL)
	{
		data.chg_psy = power_supply_get_by_name(ICHGMANAME);
		if(data.chg_psy == NULL)
		{
			pr_info("QNS: ERROR: unable to get " ICHGMANAME ". Can't set the current!");
			return false;
		}
	}
	pr_info("QNS: qns_set_ibat  CHARGE_CURRENT_PROP = %d",CHARGE_CURRENT_PROP);
	if(power_supply_set_property(data.chg_psy,
			CHARGE_CURRENT_PROP, &propVal) != 0)
	{
		pr_info("QNS: ERROR: unable to set charging current! Does " ICHGMANAME " have "
				"POWER_SUPPLY_PROP_VOLTAGE_AVG property?");
		return QNS_ERROR;
	}
	return QNS_OK;
}
...
static int qns_set_vbat(int vbatmV)
{
	...
	if(data.chg_psy == NULL)
	{
		...
		if(data.chg_psy == NULL)
		{
			pr_info("QNS: ERROR: unable to get " ICHGMANAME ". Can't set the voltage!");
			return false;
		}
	}
	...
	{
		pr_info("QNS: ERROR: unable to set charging voltage! Does " ICHGMANAME " have "
				"POWER_SUPPLY_PROP_VOLTAGE_AVG property?");
		return QNS_ERROR;
	}
	return QNS_OK;
