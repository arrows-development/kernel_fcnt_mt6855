...
#define RECHG_THRESHOLD		200
#define	MTK_PD_CONNECT_PE_READY_SNK_PD30 3
#define	MTK_PD_CONNECT_PE_READY_SNK_APDO 4
#define OTG_CURRENT 1100
...
static int mt6375_chg_get_otg_current(struct mt6375_chg_data *ddata)
{
	int ret = 0;
	u32 val = 0;
	u32 val_time = 0;
	ret = mt6375_chg_field_get(ddata, F_OTG_CC, &val);
	if (ret < 0)
		return ret;
	dev_err(ddata->dev, "mt6375_chg_get_otg_current = %d\n",val);

	ret = mt6375_chg_field_get(ddata, F_CHG_TMR, &val_time);
	if (ret < 0)
		return ret;
	dev_err(ddata->dev, "mt6375_chg_get_charger_time = %d\n",val_time);
	return 0;
}
				   ...
				   union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_USB_TYPE:
		...
		if((pd_type == MTK_PD_CONNECT_PE_READY_SNK_PD30 || pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO) && val->intval == POWER_SUPPLY_USB_TYPE_SDP) {
			val->intval = POWER_SUPPLY_USB_TYPE_CDP;
			dev_err(ddata->dev, "usb type c to c connect for phone or pc\n");
		}
		mutex_unlock(&ddata->attach_lock);
		break;
	...
	case POWER_SUPPLY_PROP_TYPE:
		mutex_lock(&ddata->attach_lock);
		if((pd_type == MTK_PD_CONNECT_PE_READY_SNK_PD30 || pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO) && ddata->psy_desc.type == POWER_SUPPLY_TYPE_USB) {
			ddata->psy_desc.type = 	POWER_SUPPLY_TYPE_USB_CDP;
			val->intval = ddata->psy_desc.type;
			dev_err(ddata->dev, "type c to c connect for phone or pc\n");
		}
		else
			val->intval = ddata->psy_desc.type;
		mutex_unlock(&ddata->attach_lock);
...
static int mt6375_set_pd_type(struct charger_device *chgdev, int type)
{
	pd_type = type;
	printk("mt6375_set_pd_type pd type = %d\n",pd_type);
	return 0;
}
...
static int mt6375_get_vbat(struct charger_device *chgdev, u32 *vbat)
{
	return mt6375_get_adc(chgdev, ADC_CHANNEL_VBAT, vbat, vbat);
}
...
static int mt6375_enable_otg(struct charger_device *chgdev, bool en)
{
	...
	mt_dbg(ddata->dev, "en=%d\n", en);
	ret = mt6375_chg_field_set(ddata, F_OTG_EN, en);
	return ret;
}
...
static int mt6375_dump_registers(struct charger_device *chgdev)
{
	...
	static struct {
		...
		{ .fd = F_CHG_TMR, .name = "CHG_TMR", .unit = "hr" },
		{ .fd = F_CHG_TMR_EN, .name = "CHG_TMR_EN", .unit = "" },
	};
	static struct {
		const char *name;
		const char *unit;
	...
	static struct {
		...
		{ .reg = MT6375_REG_CHG_TMR, .name = "CHG_TMR" },
	};
	struct mt6375_chg_data *ddata = charger_get_data(chgdev);
...
static const struct charger_ops mt6375_chg_ops = {
	...
	.set_pd_type = mt6375_set_pd_type,
	/* OTG */
...
static int mt6375_chg_init_setting(struct mt6375_chg_data *ddata)
{
	...
	ret = mt6375_chg_field_set(ddata, F_OTG_CC, OTG_CURRENT);
	if (ret < 0) {
		dev_err(ddata->dev, "failed to set otg current\n");
		return ret;
	}
	...
	mt6375_chg_get_otg_current(ddata);
