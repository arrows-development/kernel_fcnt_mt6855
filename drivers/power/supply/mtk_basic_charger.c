...
#define CHARGER_CURRENT_3A 3000000
#define BATTERY_VOLTAGE_4250MV 4250
#define BATTERY_VOLTAGE_4480MV 4480
#define BATTERY_CALL_MODE_CURRENT 1200000
#define BATTERY_CAPACITY_FULL 100
#define BATTERY_CAPACITY_RECHG 97
#define FV_REDUCTION_VOLTAGE 4450000
#define DEFAULT_IEOC_300MA 300000
static g_chg_cycle_count = -1;
extern int bm_flag;
extern bool fv_flag;
extern int battery_voltage_adc;
extern int charger_voltage;
extern int battery_current;
int charging_current_limit = 0;

int get_charging_current_limit(void) {
	return charging_current_limit;
}
EXPORT_SYMBOL(get_charging_current_limit);
...
static void select_cv(struct mtk_charger *info)
{
	...
	if (info->qns_cv != 0) {
		if (info->qns_cv < constant_voltage)
			constant_voltage = info->qns_cv;
		printk("[%s]: constant_voltage =%d,qns_cv = %d\n",__func__,constant_voltage,info->qns_cv);
	}

	if(fv_flag == true) {
		info->setting.cv = FV_REDUCTION_VOLTAGE;
	} else {
		info->setting.cv = constant_voltage;
	}

	if (info->sw_jeita.ieoc != 0){
		info->setting.ieoc = info->sw_jeita.ieoc;
	} else {
		info->setting.ieoc = DEFAULT_IEOC_300MA;
	}
}
	...
	struct chg_limit_setting *setting)
{
	...
	if (info->chr_type == POWER_SUPPLY_TYPE_USB &&
		...
		pdata->input_current_limit =
			info->data.pd_charger_input_current;
		if(charger_voltage <= 6000)
			pdata->input_current_limit  = CHARGER_CURRENT_3A;
	...
	if (info->enable_sw_jeita) {
		if (info->chr_type == POWER_SUPPLY_TYPE_USB)
			...
			if (info->sw_jeita.sm == TEMP_T0_TO_T1) {
				chr_err("pd_type TEMP_T0_TO_T1 battery_voltage_adc = %d,battery_current = %d\n",battery_voltage_adc,battery_current);
				...
				chr_err("pd_type TEMP_T1_TO_T2 battery_voltage_adc = %d,battery_current = %d\n",battery_voltage_adc,battery_current);
				pdata->charging_current_limit = 3000000;
 				if ((battery_voltage_adc >= BATTERY_VOLTAGE_4250MV) && (battery_current <= CHARGER_CURRENT_1A/1000)){
					pdata->charging_current_limit = 1000000;
				}
				...
				pdata->charging_current_limit = 2450000;
			}
			chr_err("pd_type get jeita charging_current = %d\n",pdata->charging_current_limit);
		} else if (info->chr_type == POWER_SUPPLY_TYPE_USB_DCP) {
			if (info->sw_jeita.sm == TEMP_T0_TO_T1) {
				chr_err("dcp_type TEMP_T0_TO_T1 battery_voltage_adc = %d,battery_current = %d\n",battery_voltage_adc,battery_current);
				pdata->charging_current_limit = 2000000;
 				if ((battery_voltage_adc >= BATTERY_VOLTAGE_4250MV) && (battery_current <= CHARGER_CURRENT_1A/1000)){
					pdata->charging_current_limit = 1000000;
				}
				...
				pdata->charging_current_limit = 2000000;
				chr_err("dcp_type TEMP_T1_TO_T2 battery_voltage_adc = %d,battery_current = %d\n",battery_voltage_adc,battery_current);
				if ((battery_voltage_adc >= BATTERY_VOLTAGE_4250MV) && (battery_current <= CHARGER_CURRENT_1A/1000)){
					pdata->charging_current_limit = 1000000;
				}
			} else if (info->sw_jeita.sm == TEMP_T2_TO_T3) {
				...
				pdata->charging_current_limit = 2000000;
			chr_err("dcp_type get jeita charging_current = %d\n",pdata->charging_current_limit);
		} else  { //for cdp
			if (info->sw_jeita.sm == TEMP_T0_TO_T1) {
				chr_err("cdp_type TEMP_T0_TO_T1 battery_voltage_adc = %d,battery_current = %d\n",battery_voltage_adc,battery_current);
				pdata->charging_current_limit = 1500000;
				if ((battery_voltage_adc >= BATTERY_VOLTAGE_4250MV) && (battery_current <= CHARGER_CURRENT_1A/1000)){
					pdata->charging_current_limit = 1000000;
				}
			} else if (info->sw_jeita.sm == TEMP_T1_TO_T2) {
				chr_err("cdp_type TEMP_T1_TO_T2 battery_voltage_adc = %d,battery_current = %d\n",battery_voltage_adc,battery_current);
				pdata->charging_current_limit = 1500000;
				if ((battery_voltage_adc >= BATTERY_VOLTAGE_4250MV) && (battery_current <= CHARGER_CURRENT_1A/1000)){
					pdata->charging_current_limit = 1000000;
				}
			...
			chr_err("cdp_type get jeita charging_current = %d\n",pdata->charging_current_limit);
		}
	}

	befor_current_limit = pdata->charging_current_limit;
	if(info->call_state) {
		if (info->chr_type == POWER_SUPPLY_TYPE_USB)
			chr_debug("USBIF & STAND_HOST skip current check\n");
		else
			pdata->charging_current_limit = BATTERY_CALL_MODE_CURRENT;
	} else
		pdata->charging_current_limit = befor_current_limit;
	...
	if (pdata->thermal_charging_current_limit != -1) {
		if (pdata->thermal_charging_current_limit <=
			...
			info->setting.charging_current_limit1 =
					pdata->thermal_charging_current_limit;
			if(!info->led_status) {
				if (pdata->thermal_charging_current_limit == 600000) {//>=42
					pdata->charging_current_limit = 2000000;
					chr_err("thermal temp above 42,set charging_current_limit = %d\n",
						pdata->charging_current_limit);
					if(pdata->charging_current_limit > befor_current_limit){
						pdata->charging_current_limit = befor_current_limit;
						chr_err("thermal temp below 40,set charging_current_limit = %d\n",
							pdata->charging_current_limit);
					}
				} else if(pdata->thermal_charging_current_limit == 400000) {//>=44
					pdata->charging_current_limit = 1500000;
					chr_err("thermal temp above 44,set charging_current_limit = %d\n",
						pdata->charging_current_limit);
					if(pdata->charging_current_limit > befor_current_limit){
						pdata->charging_current_limit = befor_current_limit;
						chr_err("thermal temp below 42,set charging_current_limit = %d\n",
							pdata->charging_current_limit);
					}
				} else if(pdata->thermal_charging_current_limit == 300000) {//>=46
					pdata->charging_current_limit = 1000000;
					chr_err("thermal temp above 46,set charging_current_limit = %d\n",
						pdata->charging_current_limit);
					if(pdata->charging_current_limit > befor_current_limit){
						pdata->charging_current_limit = befor_current_limit;
						chr_err("thermal temp below 44,set charging_current_limit = %d\n",
							pdata->charging_current_limit);
					}
				} else {
					pdata->charging_current_limit = befor_current_limit;
	...
	if (info->qns_charging_current != 0) {
		if (info->qns_charging_current < pdata->charging_current_limit) {
			pdata->charging_current_limit = info->qns_charging_current;
			chr_err("select_charging_current_limit qns_charging_current = %d\n",
			info->qns_charging_current);
		}
	}
	charging_current_limit = pdata->charging_current_limit;
...
static int do_algorithm(struct mtk_charger *info)
{
	...
	if (info->is_chg_done != chg_done) {
		if (chg_done) {
			if(get_uisoc(info) == BATTERY_CAPACITY_FULL){
				info->rechg_flag = true;
			}
			charger_dev_do_event(info->chg1_dev, EVENT_FULL, 0);
			info->polling_interval = CHARGING_FULL_INTERVAL;
			chr_err("%s battery full\n", __func__);
		} else {
			if (get_uisoc(info) > BATTERY_CAPACITY_RECHG) {
				info->hw_rechg_flag = true;
				chr_err("%s hw recharge,set battery full!\n", __func__);
			} else {
	...
	if (is_basic == true) {
		...
		chr_err("%s:old_ieoc=%d,ieoc=%d\n",
			__func__,
			info->old_ieoc,
			info->setting.ieoc);
		if (info->old_ieoc == 0 || (info->old_ieoc != info->setting.ieoc)){
			charger_dev_set_eoc_current(info->chg1_dev,
				info->setting.ieoc);
			info->old_ieoc = info->setting.ieoc;
		}
	}
