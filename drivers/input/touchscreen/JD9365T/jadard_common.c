...
struct proc_dir_entry *pjadard_android_touch_proc_dir = NULL;
...
#define LCM_VENDOR_JD9365T_NAME	"jd9365t_fhdp_dsi_vdo_lce_3"
...
bool jd_probe_complete;
EXPORT_SYMBOL(jd_probe_complete);
...
char *jd_i_CTPM_lc_name = "Yamato_LCE_MDT_JD0111.bin";
char *jd_i_CTPM_bn_name = "Yamato_LCE_MDT_JD0301.bin";
char *jd_i_CTPM_firmware_name = NULL;
#endif
...
#define JADARD_PROC_SORTING_TEST_FILE  "self_test"
...
int jadard_distribute_touch_data(struct jadard_ts_data *ts, uint8_t *buf, int irq_event, int ts_status)
{
    ...
    case JD_REPORT_COORD:
        ...
        /* Copy touch stylus info. */
        if (pjadard_ic_data->JD_STYLUS_EN) {
            memcpy(pReport_data->touch_stylus_info, buf + JD_TOUCH_STYLUS_INFO_ADDR,
...
const char *jd_psy_name[2] = {
    "mtk-master-charger", /* Computer type */
    "ac"   /* Adapter type */
};
                                    ...
                                    size_t len, loff_t *pos)
{
    ...
    if (!pjadard_debug->proc_send_flag) {
        ...
        if (buf_tmp != NULL) {
            ...
            for (i = 2; i < 8; i++) {
                if (((val >> i) & 0x01) == 0x01) { /* Fail */
                    if (i == 7) {
                         ret += scnprintf(buf_tmp + ret, len - ret, "%dF\n", i - 2);
                    } else {
                        ret += scnprintf(buf_tmp + ret, len - ret, "%dF-", i - 2);
                    }
                } else { /* Pass */
                    if (i == 7) {
                        ret += scnprintf(buf_tmp + ret, len - ret, "%dP\n", i - 2);
                    } else {
                        ret += scnprintf(buf_tmp + ret, len - ret, "%dP-", i - 2);
                    }
                }
            }
...
int jadard_common_proc_init(void)
{
    ...
    pjadard_android_touch_proc_dir = proc_mkdir("android_touch", NULL);
#if defined(JD_OPPO_FUNC)
    ...
    jadard_proc_sorting_test_file = proc_create(JADARD_PROC_SORTING_TEST_FILE, (S_IRUGO),
                                        pjadard_android_touch_proc_dir, &jadard_proc_sorting_test_ops);
...
static void jadard_esd_check(struct work_struct *work)
{
    while(1) {
        JD_I("ESD check\n");
        if ((pjadard_ts_data->suspended == false) && (pjadard_ts_data->fw_ready == true) &&
...
static int jdtp_project_id = -1; //  null=-1,lc=0,bn=1

static void jadard_check_dt(void)
{
    ...
    if (lcm_name) {
        videolfb_tag = (struct tag_videolfb *)of_get_property(lcm_name,
                        ...
                        (int *)&size);
        if (!videolfb_tag){
            JD_E("videolfb_tag error!\n");
        }
    	JD_I("%s: videolfb_tag->lcmname = %s\n", __func__, videolfb_tag->lcmname);
    }else{
        JD_I("%s: lcm_name error!\n", __func__);
    }

    if (strncmp(videolfb_tag->lcmname, LCM_VENDOR_JD9365T_NAME,
	strlen(LCM_VENDOR_JD9365T_NAME))) {
	jd_i_CTPM_firmware_name = jd_i_CTPM_lc_name;
	jdtp_project_id = 0;
	JD_I("%s: lcm panel is 'lc'!\n", __func__);
    } else {
	jd_i_CTPM_firmware_name = jd_i_CTPM_bn_name;
	jdtp_project_id = 1;
	JD_I("%s: lcm panel is 'bn'!\n", __func__);
    }
}

int get_jdtp_project_id(void)
{
	return jdtp_project_id;
}
EXPORT_SYMBOL(get_jdtp_project_id);
...
int jadard_chip_common_init(void)
{
    ...
    int jdtp_boot_mode = -1;
    jdtp_boot_mode = jdtp_parse_boot_mode(&ts->spi->controller->dev);
    JD_I("%s: boot_mode = %d\n", __func__, jdtp_boot_mode);
    if (jdtp_boot_mode == 8) {
        JD_E("%s:  off charging mode, return\n", __func__);
        return 0;
    }
    ...
    ts->pinctrl = devm_pinctrl_get(ts->spi->controller->dev.parent);
    if (IS_ERR_OR_NULL(ts->pinctrl)) {
        JD_E("Failed to get pinctrl, please check dts, %llu\n", ts->pinctrl);
        err = PTR_ERR(ts->pinctrl);
        return err;
    }
    ts->pin_spi_mode_default = pinctrl_lookup_state(ts->pinctrl, "ctp_spi_mode");
    if (IS_ERR(ts->pin_spi_mode_default)) {
            err = PTR_ERR(ts->pin_spi_mode_default);
            JD_E("Cannot find pinctrl state_spi_mode!\n");
            return err;
    }
    err = pinctrl_select_state(ts->pinctrl, ts->pin_spi_mode_default);
    if(err <0 ){
        JD_E("Set  pin spi mode error:%d", err);
    }
