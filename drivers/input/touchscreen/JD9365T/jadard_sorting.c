...
#else
    JD_I("%s : Kernel5.10-user not support kernel_write.");
#endif
#else
    kernel_write(raw_file, rslt_buf, JD_OUTPUT_BUFFER_SIZE * JD_SORTING_ACTIVE_ITEM * sizeof(char), pos);
#endif
...
static void jadard_Start_to_Get_IdleDiffData_Function(void)
{
    int half_rawdata_len = pjadard_ic_data->JD_X_NUM * pjadard_ic_data->JD_Y_NUM * sizeof(uint8_t);

    JD_I("Start to Get Cascade Diff Data\n");
    ...
    if (Fail_flag == true) {
        ...
        if (g_module_fp.fp_GetSortingDiffData) {
            ...
            if (g_module_fp.fp_GetSortingIdleDiffDataMax && g_module_fp.fp_GetSortingIdleDiffDataMin) {
                g_module_fp.fp_GetSortingIdleDiffDataMax(GET_RAWDATA, half_rawdata_len);
                g_module_fp.fp_GetSortingIdleDiffDataMin(GET_RAWDATA + half_rawdata_len, half_rawdata_len);
#else
            if (g_module_fp.fp_GetSortingIdleDiffDataMax) {
                g_module_fp.fp_GetSortingIdleDiffDataMax(GET_RAWDATA, half_rawdata_len * 2);
#endif
            } else {
...
static uint32_t jadard_TestItem_Select(uint8_t sorting_item, bool retry_flag)
{
    ...
    case JD_SORTING_NORMALIDLE_NOISE:
        ...
        if(pjadard_ic_data->JD_MODULE_CASCADE_MODE == JD_CASCADE_MODE_ENABLE) {
            jadard_Start_to_Get_IdleDiffData_Function();
        } else {
            jadard_Start_to_Get_DiffData_Function();
        }
        if (Fail_flag == true) {
...
static int jadard_parse_sorting_threshold_file(void)
{
    ...
    char *file_name = "Yamato_LCE_MDT_JD0103.txt";
    char **result = NULL;
...
#ifdef JD_UPGRADE_ITO_FW
    fileName = jd_i_CTPM_firmware_name;
    JD_I("%s: Bin file name(%s)\n", __func__, fileName);
