...
static void jd9365t_ChipInfoInit(void)
{
    ...
    g_jd9365t_section_info.section_info_ready            = false;
    g_jd9365t_section_info.section_info_ready_addr       = JD9365T_MEMORY_ADDR_ERAM;

    g_jd9365t_section_info.dsram_num_start_addr          = JD9365T_MEMORY_ADDR_ERAM + 2;
    g_jd9365t_section_info.dsram_section_info_start_addr = JD9365T_MEMORY_ADDR_ERAM + 4;

    g_jd9365t_section_info.esram_num_start_addr          = (g_jd9365t_section_info.dsram_section_info_start_addr +
                                                            JD9365T_MAX_DSRAM_NUM * sizeof(struct JD9365T_INFO_CONTENT));
    g_jd9365t_section_info.esram_section_info_start_addr = g_jd9365t_section_info.esram_num_start_addr + 4;
}
...
static int jd9365t_PorInit(void)
{
    int ReCode;

    ReCode = jd9365t_WriteRegSingle((uint32_t)JD9365T_SOC_REG_ADDR_POR_INIT,
                                        (uint8_t)JD9365T_SOC_PASSWORD_POR_CLEAR);
...
static int jd9365t_DisableWDTRun(void)
{
    int ReCode;

    ReCode = jd9365t_WriteRegSingle((uint32_t)JD9365T_TIMER_REG_ADDR_WDT_CONFIG,
                                    (uint8_t)JD9365T_TIMER_RELATED_SETTING_WDT_STOP);
...
static bool jd9365t_GetSectionFirstValueStatus(uint16_t usTimeOut)
{
    ...
    do {
        ...
        mdelay(2);
        if (jd9365t_ReadRegMulti(g_jd9365t_section_info.section_info_ready_addr, rbuf, sizeof(rbuf)) < 0) {
...
static int jd9365t_GetSectionInfo(uint32_t start_addr, uint8_t section_num, struct JD9365T_INFO_CONTENT *info_content)
{
    ...
    ReCode = jd9365t_ReadRegMulti(start_addr, rbuf, rbuf_len);
    if (ReCode < 0) {
        JD_E("%s: Read section info fail\n", __func__);
        kfree(rbuf);
        return ReCode;
...
static int jd9365t_ReadSectionInfo(bool reinit_config)
{
    int ReCode, i;
    ...
    if (jd9365t_GetSectionFirstValueStatus(50) == true) {
        ...
        jd9365t_GetSectionInfo(g_jd9365t_section_info.dsram_section_info_start_addr,
                                JD9365T_PSRAM_SORT_PARA + 1, &dsram_info_content[0]);

        jd9365t_GetSectionInfo(g_jd9365t_section_info.esram_section_info_start_addr,
        ...
        if (ReCode < 0) {
            ...
            g_jd9365t_chip_info.dsram_host_addr.proximity_en =
                dsram_info_content[JD9365T_DSRAM_HOST].info_content_addr + 44;
            g_jd9365t_chip_info.dsram_host_addr.panel_maker =
                dsram_info_content[JD9365T_DSRAM_HOST].info_content_addr + 56;
            g_jd9365t_chip_info.dsram_host_addr.panel_version =
                dsram_info_content[JD9365T_DSRAM_HOST].info_content_addr + 57;
            g_jd9365t_chip_info.dsram_host_addr.earphone_en =
                dsram_info_content[JD9365T_DSRAM_HOST].info_content_addr + 58;
            ...
            g_jd9365t_chip_info.dsram_host_addr.fw_dbic_en =
                dsram_info_content[JD9365T_DSRAM_HOST].info_content_addr + 54;
            g_jd9365t_chip_info.dsram_host_addr.mpap_pw_sync =
                dsram_info_content[JD9365T_DSRAM_HOST].info_content_addr + 76;
            g_jd9365t_chip_info.dsram_host_addr.mpap_error_msg =
                dsram_info_content[JD9365T_DSRAM_HOST].info_content_addr + 78;
...
#endif
            g_jd9365t_chip_info.dsram_host_addr.data_output_v2 =
                dsram_info_content[JD9365T_DSRAM_HOST].info_content_addr + 84;
...
{
    return jd9365t_WriteRegSingle(g_jd9365t_section_info.section_info_ready_addr, 0x00);
}
...
static int jd9365t_AbortHostDMA(void)
{
    uint8_t wBuf[JD_ONE_SIZE];

    wBuf[0] = (uint8_t)JD9365T_DMA_RELATED_SETTING_DMA_ABORT;

    return jd9365t_WriteRegMulti((uint32_t)JD9365T_FLASH_REG_ADDR_DMA_0ABORT, wBuf, sizeof(wBuf));
}
...
static void jd9365t_GetHeaderInfo(uint8_t *pFileData, struct JD9365T_MOVE_INFO *move_info, int move_info_sz)
{
    ...
    for (i = 0; i < move_info_sz; i++) {
        ...
        move_info[i].fl_len =
            (uint32_t)((pFileData[header_shift + i * JD9365T_HW_HEADER_MOVE_INFO_NUMBER + 4] << 8) +
...
static uint8_t jd9365t_GetHeaderPsramIndex(struct JD9365T_MOVE_INFO *move_info, int move_info_sz)
{
    int i;
    uint8_t index = 0;

    for (i = 0; i < move_info_sz; i++) {
        if (move_info[i].to_mem_st_addr == JD9365T_HW_HEADER_PSRAM_START_ADDR) {
            index = (uint8_t)i;
            break;
        }
    }
...
static uint8_t jd9365t_GetMoveInfoPsramNumber(struct JD9365T_MOVE_INFO *move_info, int move_info_sz)
{
    int i;
    uint8_t num = 0;

    for (i = 0; i < move_info_sz; i++) {
        if (move_info[i].to_mem_st_addr <= JD9365T_PRAM_MAX_SIZE)
            num++;
    }
...
static int jd9365t_HostWriteFlash(uint32_t WriteAddr, uint8_t *pFileData, uint32_t FileSize)
{
    ...
    do {
        ...
        MoveInfoPsramNumber = jd9365t_GetMoveInfoPsramNumber(MoveInfo, MoveInfoNumber);
        MoveInfoPsramIndex = jd9365t_GetHeaderPsramIndex(MoveInfo, MoveInfoNumber);
        for (i = 0; i < MoveInfoPsramNumber; i++) {
            jd9365t_ReArrangePageProgramInfo((uint32_t)(MoveInfo[MoveInfoPsramIndex + i].fl_st_addr - JD9365T_HW_HEADER_MAX_SIZE),
                                            (uint32_t)(JD9365T_HW_HEADER_MAX_SIZE + MoveInfo[MoveInfoPsramIndex + i].fl_len),
            ...
            if (FileSize >= (PageProgram_addr + PageProgram_len)) {
                ...
                jd9365t_GetDataCRC16(crcbuffer, PageProgram_len, 0, PageProgram_len, &softcrc, JD9365T_CRC_INFO_PolynomialCRC16);
                kfree(crcbuffer);
    ...
    /* Reload section info. */
    ReCode = jd9365t_ReadSectionInfo(true);
...
#ifdef JD_ZERO_FLASH
#ifdef JD_SET_VDDD
static void jd9365t_SetVDDD(uint8_t parameter)
{
    uint8_t dd_reg_cmd;
    uint8_t wdata[4];
    ...
    /* Write password */
    dd_reg_cmd = 0xDF;
    wdata[0] = JD9365T_DD_PASSWORD_CODE1;
    wdata[1] = JD9365T_DD_PASSWORD_CODE2;
    wdata[2] = JD9365T_DD_PASSWORD_CODE3;
    if (g_module_fp.fp_dd_register_write(0x00, dd_reg_cmd, wdata, 3) == JD_DBIC_READ_WRITE_FAIL) {
    ...
    dd_reg_cmd = 0xDE;
    wdata[0] = 0x00;
    if (g_module_fp.fp_dd_register_write(0x00, dd_reg_cmd, wdata, 1) == JD_DBIC_READ_WRITE_FAIL) {
        JD_E("%s: Write display register(%x) fail!\n", __func__, dd_reg_cmd);
    }
    ...
    wdata[3] = 0x00;
    if (g_module_fp.fp_dd_register_write(0x00, dd_reg_cmd, wdata, 4) == JD_DBIC_READ_WRITE_FAIL) {
        JD_E("%s: Write display register(%x) fail!\n", __func__, dd_reg_cmd);
    }
...
static int jd9365t_HostWritePram(uint32_t WriteAddr, uint8_t *pFileData, uint32_t FileSize)
{
    ...
    if (pjadard_ts_data->power_on_upgrade == false) {
        ...
        /* Set DMA Start & Pulling DMA busy = JD9365T_DMA_RELATED_SETTING_DMA_DONE */
        ReCode = jd9365t_SetDMAStart(MoveInfo[MoveInfoPsramIndex].to_mem_st_addr, MoveInfo[MoveInfoPsramIndex].fl_len, MoveInfo[MoveInfoPsramIndex].to_mem_st_addr,
...
SKIP_UPGRADE_FW:
    ...
    ReCode = jd9365t_ReadSectionInfo(false);
...
static void module_jd9365t_chip_init(void)
{
    ...
    /* Reload section info. */
    if (jd9365t_ReadSectionInfo(false) < 0) {
        JD_E("Chip init fail\n");
    } else {
...
static void module_jd9365t_EnterBackDoor(void)
{
    jd9365t_EnterBackDoor(NULL);
}

static void module_jd9365t_ExitBackDoor(void)
{
    jd9365t_ExitBackDoor();
}
...
static void module_jd9365t_ResetMCU(void)
{
    jd9365t_ResetMCU();
}

static void module_jd9365t_PorInit(void)
{
    jd9365t_PorInit();
}
...
static void module_jd9365t_read_fw_ver(void)
{
    ...
    ReCode_maker = module_jd9365t_register_read(g_jd9365t_chip_info.dsram_host_addr.panel_maker, rdata_maker, sizeof(rdata_maker));
    ReCode_panel = module_jd9365t_register_read(g_jd9365t_chip_info.dsram_host_addr.panel_version, rdata_panel, sizeof(rdata_panel));
    ...
    JD_I("PANEL_VER: %02x\n", pjadard_ic_data->panel_ver);
     if(jd_probe_complete) {
			sprintf(tp_version_info, "[Vendor]LCE,[TP-IC]JD9365T,[FW]%08x\n",  pjadard_ic_data->fw_cid_ver);
			hq_regiser_hw_info(HWID_CTP, tp_version_info);
			JD_I("hq_regiser_hw_info complete!");
			jd_probe_complete = false;
	}
}
...
static void module_jd9365t_mutual_data_set(uint8_t data_type)
{
    ...
    if (pjadard_ts_data->debug_diag_apk_enable) {
        status[0] = 0x55;
        status[1] = 0xAA;
        jd9365t_WriteRegMulti(g_jd9365t_chip_info.dsram_host_addr.data_output_v2, status, sizeof(status));
    }
    else {
        status[0] = 0x00;
        status[1] = 0x00;
...
static int module_jd9365t_read_mutual_data(uint8_t *rdata, uint16_t rlen)
{
    int ReCode;

    if (g_jd9365t_chip_info.dsram_debug_addr.output_data_addr > g_jd9365t_chip_info.esram_info_content_addr.coordinate_report) {
        ReCode = jd9365t_ReadRegMulti(g_jd9365t_chip_info.esram_info_content_addr.coordinate_report, rdata, rlen);
        if (ReCode >= 0)
...
static void module_jd9365t_set_rotate_border(uint16_t rotate)
{
    uint8_t wdata[JD_TWO_SIZE] = {0x00, 0x00};
    wdata[0] = (uint8_t)((rotate & 0x00FF) >> 0);
    wdata[1] = (uint8_t)((rotate & 0xFF00) >> 8);
    module_jd9365t_register_write(g_jd9365t_chip_info.dsram_host_addr.border_en, wdata, sizeof(wdata));
}

#ifdef JD_EARPHONE_DETECT
static void module_jd9365t_set_earphone_enable(uint8_t status)
{
    uint8_t wdata[JD_TWO_SIZE] = {0x00, 0x00};
    ...
    module_jd9365t_register_write(g_jd9365t_chip_info.dsram_host_addr.earphone_en, wdata, sizeof(wdata));
}
#endif
...
static void module_jd9365t_set_virtual_proximity(bool enable)
{
    uint8_t wdata[JD_TWO_SIZE] = {0x00, 0x00};
    ...
    module_jd9365t_register_write(g_jd9365t_chip_info.dsram_host_addr.proximity_en, wdata, sizeof(wdata));
}

static uint8_t module_jd9365t_ReadDbicPageEn(void)
{
    uint8_t rdata = 0;

    if (module_jd9365t_register_read((uint32_t)JD9365T_SOC_REG_ADDR_DBIC_PAGE_EN, &rdata, 1) < 0) {
        JD_E("%s: Read dbic page enable fail\n", __func__);
        return 0;
    } else {
        JD_I("%s: DBIC_PAGE_EN = %d\n", __func__, rdata);
        return rdata;
    }
}

static int module_jd9365t_SetDbicPage(uint8_t page)
{
    int ReCode;
    uint8_t wdata[JD_ONE_SIZE];

    wdata[0] = page & 0x07;
    ReCode = module_jd9365t_register_write((uint32_t)JD9365T_SOC_REG_ADDR_DBIC_PAGE, wdata, sizeof(wdata));

    if (ReCode < 0) {
        JD_E("%s: Set dbic page fail\n", __func__);
    }
...
#ifdef CONFIG_TOUCHSCREEN_JADARD_SORTING
static void jd9365t_SetMpapPwSync(uint8_t value)
{
    uint8_t wdata[JD_TWO_SIZE] = {0x00, 0x00};

    wdata[0] = value;
    module_jd9365t_register_write(g_jd9365t_chip_info.dsram_host_addr.mpap_pw_sync, wdata, sizeof(wdata));
}

static void module_jd9365t_APP_SetSortingMode(uint8_t *value, uint8_t value_len)
{
    jd9365t_SetMpapPwSync(0);
    module_jd9365t_register_write(g_jd9365t_chip_info.dsram_host_addr.mpap_pw, value, value_len);
    jd9365t_SetMpapPwSync(1);
}
...
static bool module_jd9365t_APP_ReadSortingBusyStatus(uint8_t mpap_handshake_finish, uint8_t *pStatus)
{
...
static void module_jd9365t_Fw_DBIC_Off(void)
{
    uint8_t wBuf[JD_TWO_SIZE];

    wBuf[0] = 0x24;
    wBuf[1] = 0x02;
    module_jd9365t_register_write(g_jd9365t_chip_info.dsram_host_addr.fw_dbic_en, wBuf, sizeof(wBuf));
}

static void module_jd9365t_Fw_DBIC_On(void)
{
    uint8_t wBuf[JD_TWO_SIZE];
...
static void module_jd9365t_ReadMpapErrorMsg(uint8_t *pValue, uint8_t pValue_len)
{
    module_jd9365t_register_read(g_jd9365t_chip_info.dsram_host_addr.mpap_error_msg, pValue, pValue_len);
}
#endif
...
{
    JD_D("%s:Entering!\n", __func__);
    g_module_fp.fp_EnterBackDoor        = module_jd9365t_EnterBackDoor;
    g_module_fp.fp_ExitBackDoor         = module_jd9365t_ExitBackDoor;
    g_module_fp.fp_pin_reset            = module_jd9365t_PinReset;
    g_module_fp.fp_soft_reset           = module_jd9365t_SoftReset;
    g_module_fp.fp_ResetMCU             = module_jd9365t_ResetMCU;
    g_module_fp.fp_PorInit              = module_jd9365t_PorInit;
    ...
    g_module_fp.fp_read_mutual_data     = module_jd9365t_read_mutual_data;
    g_module_fp.fp_set_high_sensitivity = module_jd9365t_set_high_sensitivity;
    g_module_fp.fp_set_rotate_border    = module_jd9365t_set_rotate_border;
#ifdef JD_EARPHONE_DETECT
    g_module_fp.fp_set_earphone_enable  = module_jd9365t_set_earphone_enable;
#endif
    ...
    g_module_fp.fp_set_virtual_proximity = module_jd9365t_set_virtual_proximity;
    g_module_fp.fp_ReadDbicPageEn        = module_jd9365t_ReadDbicPageEn;
    g_module_fp.fp_SetDbicPage           = module_jd9365t_SetDbicPage;
#ifdef CONFIG_TOUCHSCREEN_JADARD_SORTING
    ...
    g_module_fp.fp_Fw_DBIC_Off               = module_jd9365t_Fw_DBIC_Off;
    g_module_fp.fp_Fw_DBIC_On                = module_jd9365t_Fw_DBIC_On;
    g_module_fp.fp_StartMCU                  = module_jd9365t_StartMCU;
    g_module_fp.fp_ReadMpapErrorMsg          = module_jd9365t_ReadMpapErrorMsg;
#endif
}
...
static bool module_jd9365t_chip_detect(void)
{
    ...
    g_common_variable.dbi_dd_reg_mode = JD9365T_DDREG_MODE;
    ...
    do {
        ...
        if (romid == CHIP_ID) {
            JD_I("%s: detect IC %s successfully\n", __func__, JD9365T_OUTER_ID);
            scnprintf(pjadard_ic_data->chip_id, sizeof(pjadard_ic_data->chip_id), JD9365T_OUTER_ID);
            jd9365t_func_reinit();
            ret = true;
        } else {
...
static int jadard_jd9365t_probe(struct platform_device *pdev)
{
    ...
    pChip->chip_init = module_jd9365t_chip_init;
    pChip->chip_detect = module_jd9365t_chip_detect;
    pChip->next = NULL;
...
static int jadard_jd9365t_remove(struct platform_device *pdev)
{
    ...
    while (ptr != NULL) {
        if ((ptr->chip_init == module_jd9365t_chip_init) &&
            (ptr->chip_detect == module_jd9365t_chip_detect)) {
