...
struct proc_ops gcore_app_fops = {
...
long gcore_app_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case IOC_APP_DEMO_RAWDATA:
		...
		break;
/*
1.Register reading: The application layer can obtain the underlying register information through the ioctl (fd, IOP_APP READ REG,&reg_info) method, 
with parameter Reg_ Info is a struct reg_ Msg structure.
*/
	case IOC_APP_READ_REG:
		...
		break;
/*
2.Register writing: The application layer can write low-level register information using the ioctl (fd, IOP_APP_WRITE-REG,&reg_info) method, 
with the parameter reg_ Info is a struct reg_ Msg structure.
*/
...
/*
3.Switching Mode: The application layer can switch the underlying running mode by using the ioctl (fd, IOP_APP_SWITCH-MODE,&mode_info) method, 
with the parameter mode_ Info is a struct mode_ Inf structure.
*/
...
struct proc_ops gcore_selftest_fops = {
