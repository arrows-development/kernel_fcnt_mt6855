...
#define FW_BIN_NAME "Yamato_LD_MDT_GC0210.bin"
#define FW_BIN_NAME_ID1 "Yamato_LD_MDT_GC04A1.bin"

#define STARTUP_UPDATE_DELAY_MS  1000	/* ms */

#define LCM_VENDOR_GC7202M_NAME	"gc7202m_fhdp_dsi_vdo_lide_4"
...
static bool lcm_name_flag = false; // true=bn false=lc
static int gctp_project_id = -1;// null=-1,lc=0,bn=1

static void gcore_check_dt(void)
{
    ...
    if (lcm_name) {
        videolfb_tag = (struct tag_videolfb *)of_get_property(lcm_name,
                        ...
                        (int *)&size);
        if (!videolfb_tag){
            GTP_DEBUG("videolfb_tag error!\n");
        }
        GTP_DEBUG("%s: videolfb_tag->lcmname = %s\n", __func__, videolfb_tag->lcmname);
    }else{
        GTP_DEBUG("%s: lcm_name error!\n", __func__);
    }
	

    if (strncmp(videolfb_tag->lcmname, LCM_VENDOR_GC7202M_NAME,
        strlen(LCM_VENDOR_GC7202M_NAME))) {
        GTP_DEBUG("%s: lcm panel is 'lc'!\n", __func__);
	gctp_project_id = 0;
    } else {
        GTP_DEBUG("%s: lcm panel is 'bn'!\n", __func__);
	gctp_project_id = 1;
	lcm_name_flag = true;
    }
}

int get_gctp_project_id(void)
{
	return gctp_project_id;
}
EXPORT_SYMBOL(get_gctp_project_id);

void gcore_request_firmware_update_work(struct work_struct *work)
{
	int ret = 0;
	static bool check_dt_flag = false;
	...
	if (lcm_name_flag) {
		ret = request_firmware(&fw, FW_BIN_NAME_ID1, &gdev_fwu->bus_device->dev);
		GTP_DEBUG("firmware name = %s\n", FW_BIN_NAME_ID1);
	}
	else {
		ret = request_firmware(&fw, FW_BIN_NAME, &gdev_fwu->bus_device->dev);
		GTP_DEBUG("firmware name = %s\n", FW_BIN_NAME);
	}
	...
	if(probe_complete) {
		sprintf(tp_version_info, "[Vendor]LIDE,[TP-IC]GC7202M,[FW]%x %x %x %x\n",  gdev_fwu->fw_ver_in_bin[1], gdev_fwu->fw_ver_in_bin[0], \
			gdev_fwu->fw_ver_in_bin[3], gdev_fwu->fw_ver_in_bin[2]);
		hq_regiser_hw_info(HWID_CTP, tp_version_info);
		GTP_DEBUG("hq_regiser_hw_info complete!");
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (enable) {
		...
		gc_gesture_flag = true;
	} else {
		gdev_fwu->gesture_wakeup_en = false;
		gc_gesture_flag = false;
	}
				  ...
				  const char *buf, size_t count)
{
  ...
  int enabled = 0;
	...
	char spi_wbuff[] = {0x55,0xaa,0xff,0x77,0xaa,0x55,0xff,0x11};
	char spi_rbuff[8];
