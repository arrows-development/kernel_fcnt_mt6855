	...
	if(gdev->PS_Enale == true){
		...
		if(coor_data[62] & 0x01){
			...
			if(coor_data[62] == 0x81){
				...
				GTP_DEBUG("TEL: Proximity tp turn off Screen,data[62]=0x%x",coor_data[62]);
				...
				GTP_DEBUG("TEL: default data[62]=0x%x ",coor_data[62]);
			}
			gcore_proximity_report_to_application();
		}
	}
...
static int gcore_charger_notify_call(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	psy_usb = power_supply_get_by_name("mtk-master-charger");
	if (!psy_usb) {
...
*/
	...
	if ((strcmp(psy_usb->desc->name, "mtk-master-charger") == 0)) {
		GTP_DEBUG("usb2");
...
*/
	if (!strcmp(psy_usb->desc->name, "mtk-master-charger")) {
...
/* gcore_gpio_rst_output(gdev->rst_gpio, 1); */
	...
	INIT_DELAYED_WORK(&gdev->fwu_work, gcore_request_firmware_update_work);
    	INIT_WORK(&gdev->resume_work,gcore_resume);
#if GCORE_WDT_RECOVERY_ENABLE
...
void gcore_deinit(struct gcore_dev *gdev)
{
	...
	if(gdev->CB_value)
		kfree(gdev->CB_value);
#endif
	cancel_work_sync(&fn_data.gdev->resume_work);
...
int gtp_parse_boot_mode(struct device *dev)
{
    struct device_node *boot_node = NULL;
    struct gtp_tag_bootmode *tag = NULL;
    pr_info("%s: enter\n", __func__);
    ...
    if (!boot_node) {
        ...
        tag = (struct gtp_tag_bootmode *)of_get_property(boot_node, "atag,boot", NULL);
        if (!tag) {
...
int gcore_touch_probe(struct gcore_dev *gdev)
{
	...
	int gtp_boot_mode = -1;
    gtp_boot_mode = gtp_parse_boot_mode(&gdev->bus_device->controller->dev);
	if (gtp_boot_mode == 8) {
        GTP_ERROR("%s:  off charging mode, return\n", __func__);
        return 0;
    }
	...
	if (mtk_disp_notifier_register("gcore-ts",&gdev->fb_notifier)) {
...
/*#ifdef CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM
	...
	spi_config = (struct mtk_chip_config*)gcore_spi_slave->controller_data;
	if (!spi_config) {
		pr_err("spi config is error!!!");
	}

	spi_config->cs_holdtime = 5000;
	spi_config->cs_setuptime = 5000;
