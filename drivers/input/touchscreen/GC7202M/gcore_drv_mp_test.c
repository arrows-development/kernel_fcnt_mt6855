...
#define MP_TEST_DATA_DIR              "/sdcard"
...
static char gcore_result_fail_str[4] = "0F-";
static char gcore_result_pass_str[4] = "0P-";
static int gcore_result_status[6] = {0};
static char buf_test_result[100] = {0};
...
int gcore_parse_mp_test_ini(struct gcore_mp_data *mp_data)
{
	...
	if (ret == 0) {
		...
		if(NULL == tmp){
			...
			if (sscanf(tmp, "empty_place=%d,%d,%d,%d", &num1, &num2, &num3, &num4) == 4) {
				empty_place[0] = num1 - 1;
            	empty_place[1] = num2 - 1;
				empty_place[2] = num3 - 1;
            	empty_place[3] = num4 - 1;
			}
			GTP_DEBUG("read empty_place:%d,%d,%d,%d", empty_place[0],empty_place[1],empty_place[2],empty_place[3]);
		}
			...
			const u16 *data, int rows, int cols, enum MP_DATA_TYPE types)
{
	...
	set_fs(old_fs);
#else
	GTP_DEBUG("%s : Kernel5.10-user not support kernel_write.");
#endif
#else
	ret = kernel_write(file, linebuf, len, pos);
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0)
#ifndef BUILD_USER
	GTP_DEBUG("write file use kernel_write.");
	old_fs = get_fs();
	set_fs(KERNEL_DS);
	ret = kernel_write(file, linebuf, len, &pos);
	set_fs(old_fs);
...
static char *get_date_time_str(void)
{
    ...
    u64 ns = ktime_to_ns(now_time);
    time64_t sec = div_u64(ns, NSEC_PER_SEC);
    struct tm rtc_now_time;
    static char time_data_buf[128] = { 0 };
    time64_to_tm(sec, 0, &rtc_now_time);
...
#define MP_BIN_NAME "Yamato_LD_MDT_GC0203_MP.bin"
...
int gcore_mp_test_all_result(struct gcore_mp_data *mp_data)
{
	if (mp_data->test_int_pin && mp_data->int_pin_test_result) {
		gcore_result_status[5] = -1;
		return -EPERM;
	}else{
		gcore_result_status[5] = 0;
	}

	if (mp_data->test_chip_id && mp_data->chip_id_test_result){
		gcore_result_status[4] = -1;
		return -EPERM;
	}else{
		gcore_result_status[4] = 0;
	}

	if (mp_data->test_open && mp_data->open_test_result) {
		gcore_result_status[0] = -1;
		return -EPERM;
	}else{
		gcore_result_status[0] = 0;
	}

	if (mp_data->test_short && mp_data->short_test_result) {
		gcore_result_status[1] = -1;
		return -EPERM;
	}else{
		gcore_result_status[1] = 0;
	}

	if (mp_data->test_rawdata && mp_data->rawdata_test_result) {
		gcore_result_status[2] = -1;
		return -EPERM;
	}else{
		gcore_result_status[2] = 0;
	}
	
	if (mp_data->test_noise && mp_data->noise_test_result) {
		gcore_result_status[3] = -1;
		return -EPERM;
	}else{
		gcore_result_status[3] = 0;
	}
...
int gcore_start_mp_test(void)
{
	...
	usleep_range(1000, 1100);
	memset(buf_test_result,0,sizeof(buf_test_result));
	...
	for( i=0 ;i<6 ; i++){
		if(gcore_result_status[i] == 0){
			gcore_result_pass_str[0] = i + '0';
			strncat(buf_test_result, gcore_result_pass_str, strlen(gcore_result_pass_str) + 1);
		}else{
			gcore_result_fail_str[0] = i + '0';
			strncat(buf_test_result, gcore_result_fail_str, strlen(gcore_result_fail_str) + 1);
		}
	}
	GTP_DEBUG("mp test result:%d", mp_data->all_test_result);
	GTP_DEBUG("mp test result:%s", buf_test_result);
...
struct proc_ops gcore_rawdata_fops = {
	//.proc_owner = THIS_MODULE,
...
static int gcore_ito_proc_show(struct seq_file *m, void *v)
{
	int ret = 0;
	ret = gcore_start_mp_test();
	seq_puts(m,buf_test_result);
	seq_puts(m, "\n");
	
	return 0;
}

static int gcore_ito_test_open(struct inode *inode, struct file *file)
{
	return single_open(file, gcore_ito_proc_show, NULL);
}

static ssize_t gcore_ito_test_write(struct file *file, const char __user *buffer,size_t count, loff_t *pos)
{
	return -EPERM;
}


#define GCORE_SELF_TEST "self_test"
#define GCORE_ANDROID_TOUCH "android_touch"
static const struct proc_ops touch_test_fops = {
    //.owner = THIS_MODULE,
    .proc_open = gcore_ito_test_open,
    .proc_read = seq_read,
    .proc_write = gcore_ito_test_write,
};

int gcore_android_touch_selftest_init(void)
{
	struct proc_dir_entry *dir_entry;
    dir_entry = proc_mkdir(GCORE_ANDROID_TOUCH, NULL);
    if (!dir_entry)
        return -ENOMEM;
    proc_create(GCORE_SELF_TEST, 0, dir_entry, &touch_test_fops);
    GTP_DEBUG("Created /proc/%s/%s",GCORE_ANDROID_TOUCH,GCORE_SELF_TEST);
    return 0;
}

void gcore_android_touch_selftest_deinit(void)
{
	remove_proc_entry(GCORE_SELF_TEST, NULL);
    remove_proc_entry(GCORE_ANDROID_TOUCH, NULL);
    GTP_DEBUG("Removed /proc/%s/%s", GCORE_ANDROID_TOUCH, GCORE_SELF_TEST);
}
...
int gcore_mp_test_fn_init(struct gcore_dev *gdev)
{
	...
	g_mp_data = mp_data;
	gcore_android_touch_selftest_init();
...
void gcore_mp_test_fn_remove(struct gcore_dev *gdev)
{
	...
	gcore_android_touch_selftest_deinit();
