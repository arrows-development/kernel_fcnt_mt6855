...
static int mtk_pmic_keys_probe(struct platform_device *pdev)
{
    ...
    regmap_read(keys->regmap, 0x0a10, &uval[0]);
    regmap_read(keys->regmap, 0x0a0f, &uval[1]);
    pr_err("[@debug]before w %s 0x%x:0x%x, 0x%x:0x%x\n", __func__,
                    0x0a10, uval[0], 0x0a0f, uval[1]);
    regmap_update_bits(keys->regmap, 0x0a33, 0xff, 0x29);
    regmap_update_bits(keys->regmap, 0x0a34, 0xff, 0x47);
    regmap_update_bits(keys->regmap, 0x0a10, 0xf3, 0xf0);
    regmap_update_bits(keys->regmap, 0x0a0f, 0x21, 0x21);
    regmap_update_bits(keys->regmap, 0x0a33, 0xff, 0x00);
    regmap_update_bits(keys->regmap, 0x0a34, 0xff, 0x00);
    regmap_read(keys->regmap, 0x0a10, &uval[0]);
    regmap_read(keys->regmap, 0x0a0f, &uval[1]);
    pr_err("[@debug]after w %s 0x%x:0x%x, 0x%x:0x%x\n", __func__,
                        0x0a10, uval[0], 0x0a0f, uval[1]);
	return 0;
}
