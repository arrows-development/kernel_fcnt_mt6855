...
static int hw_power_reset(struct fpc_data *fpc, int enable)
{
	...
	if(enable) {
		...
		ret = select_pin_ctl(fpc, "fpsensor_fp_cs_low");
		if(ret){
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(!strncmp(buf, "enable", strlen("enable")) && fpc->compatible_enabled != 1){
		hq_regiser_hw_info(HWID_FP, "vendor:FPC,IC:FPC1552C");
		fpsensor_log(INFO_LOG, "%s hq_regiser_hw_info set success!\n", __func__);
		...
		hq_deregister_hw_info(HWID_FP, "vendor:FPC,IC:FPC1552C");
		fpsensor_log(INFO_LOG, "%s hq_deregiser_hw_info!\n", __func__);
	}
	return count;
}
