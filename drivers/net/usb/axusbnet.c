...
void axusbnet_skb_return(struct usbnet *dev, struct sk_buff *skb)
{
	...
	if (netif_msg_rx_status(dev))
		dev_dbg(&dev->udev->dev, "< rx, len %zu, type 0x%x",
		       skb->len + sizeof(struct ethhdr), skb->protocol);
	memset(skb->cb, 0, sizeof(struct skb_data));
	status = netif_rx(skb);
	if (status != NET_RX_SUCCESS && netif_msg_rx_err(dev))
		dev_dbg(&dev->udev->dev, "netif_rx status %d", status);
}
...
void axusbnet_defer_kevent(struct usbnet *dev, int work)
{
	...
	if (!schedule_work(&dev->kevent))
		dev_err(&dev->udev->dev, "kevent %d may have been dropped", work);
	else
		dev_dbg(&dev->udev->dev, "kevent %d scheduled", work);
}
...
static void rx_submit(struct usbnet *dev, struct urb *urb, gfp_t flags)
{
	...
	if (skb == NULL) {
		...
		if (netif_msg_rx_err(dev))
			dev_dbg(&dev->udev->dev, "no rx skb");
	...
	if (netif_running(dev->net)
		...
		case -ENODEV:
			if (netif_msg_ifdown(dev))
				dev_dbg(&dev->udev->dev, "device gone");
			netif_device_detach(dev->net);
			break;
		default:
			if (netif_msg_rx_err(dev))
				dev_dbg(&dev->udev->dev, "rx submit, %d", retval);
			tasklet_schedule(&dev->bh);
			break;
		case 0:
		...
		if (netif_msg_ifdown(dev))
			dev_dbg(&dev->udev->dev, "rx: stopped");
		retval = -ENOLINK;
	}
...
#endif
{
	...
	case 0:
		if (skb->len < dev->net->hard_header_len) {
			...
			if (netif_msg_rx_err(dev))
				dev_dbg(&dev->udev->dev, "rx length %d", skb->len);
		}
		break;
	...
	case -ESHUTDOWN:		/* hardware gone */
		if (netif_msg_ifdown(dev))
			dev_dbg(&dev->udev->dev, "rx shutdown, code %d", urb_status);
		goto block;
	...
	case -EILSEQ:
		...
		if (!timer_pending(&dev->delay)) {
			...
			if (netif_msg_link(dev))
				dev_dbg(&dev->udev->dev, "rx throttle %d", urb_status);
		}
block:
		state = rx_cleanup;
		entry->urb = urb;
	...
	default:
		...
		dev->net->stats.rx_errors++;
		if (netif_msg_rx_err(dev))
			dev_dbg(&dev->udev->dev, "rx status %d", urb_status);
		break;
	}
	...
	if (netif_msg_rx_err(dev))
		dev_dbg(&dev->udev->dev, "no read resubmitted");
}
...
#endif
{
	...
	case -ESHUTDOWN:	/* hardware gone */
		if (netif_msg_ifdown(dev))
			dev_dbg(&dev->udev->dev, "intr shutdown, code %d", status);
		return;
	 ...
	 */
	default:
		dev_dbg(&dev->udev->dev, "intr status %d", status);
		break;
	}
	...
	if (status != 0 && netif_msg_timer(dev))
		dev_err(&dev->udev->dev, "intr resubmit --> %d", status);
}
...
int axusbnet_stop(struct net_device *net)
{
	...
	if (netif_msg_ifdown(dev))
		dev_info(&dev->udev->dev, "stop stats: rx/tx %ld/%ld, errs %ld/%ld",
	...
	if (!(info->flags & FLAG_AVOID_UNLINK_URBS)) {
		...
		while (!skb_queue_empty(&dev->rxq)
			...
			if (netif_msg_ifdown(dev))
				dev_dbg(&dev->udev->dev, "waited for %d urb completions",
				       temp);
		}
...
int axusbnet_open(struct net_device *net)
{
	...
	if (info->check_connect) {
		...
		if (retval < 0) {
			if (netif_msg_ifup(dev))
				dev_dbg(&dev->udev->dev, "can't open; %d", retval);
			goto done;
		}
	}
	...
	if (dev->interrupt) {
		...
		if (retval < 0) {
			if (netif_msg_ifup(dev))
				dev_err(&dev->udev->dev, "intr submit %d", retval);
			goto done;
		}
	}
	...
	if (netif_msg_ifup(dev)) {
		...
		dev_info(&dev->udev->dev, "open: enable queueing (rx %d, tx %d) mtu %d %s framing",
...
static void kevent(struct work_struct *work)
{
	...
	if (test_bit(EVENT_TX_HALT, &dev->flags)) {
		...
		if (status < 0
			...
			if (netif_msg_tx_err(dev))
				dev_err(&dev->udev->dev, "can't clear tx halt, status %d",
				       status);
		} else {
	...
	if (test_bit(EVENT_RX_HALT, &dev->flags)) {
		...
		if (status < 0
				...
				&& status != -ESHUTDOWN) {
			if (netif_msg_rx_err(dev))
				dev_err(&dev->udev->dev, "can't clear rx halt, status %d",
				       status);
		} else {
	...
	if (dev->flags)
		dev_dbg(&dev->udev->dev, "kevent done, flags = 0x%lx", dev->flags);
}
	...
	if (urb->status == 0) {
		...
		case -EILSEQ:
			if (!timer_pending(&dev->delay)) {
				...
				if (netif_msg_link(dev))
					dev_dbg(&dev->udev->dev, "tx throttle %d",
					       urb->status);
			}
		...
		default:
			if (netif_msg_tx_err(dev))
				dev_dbg(&dev->udev->dev, "tx err %d", entry->urb->status);
			break;
		}
	}
...
axusbnet_start_xmit(struct sk_buff *skb, struct net_device *net)
{
	...
	if (info->tx_fixup) {
		...
		if (!skb) {
			if (netif_msg_tx_err(dev))
				dev_dbg(&dev->udev->dev, "can't tx_fixup skb");
			goto drop;
		}
	}
	length = skb->len;
	...
	default:
		if (netif_msg_tx_err(dev))
			dev_dbg(&dev->udev->dev, "tx: submit urb err %d", retval);
		break;
	case 0:
	...
	if (retval) {
		if (netif_msg_tx_err(dev))
			dev_dbg(&dev->udev->dev, "drop, code %d", retval);
drop:
		...
		dev_dbg(&dev->udev->dev, "> tx, len %d, type 0x%x",
		       length, skb->protocol);
	}
	return NETDEV_TX_OK;
}
...
#endif
{
	...
	while ((skb = skb_dequeue(&dev->done))) {
		...
		case rx_cleanup:
			...
			continue;
		default:
			dev_dbg(&dev->udev->dev, "bogus skb state %d", entry->state);
		}
	}
	...
	/* or are we maybe short a few urbs? */
		...
		if (temp < qlen) {
			...
			if (temp != dev->rxq.qlen && netif_msg_link(dev))
				dev_dbg(&dev->udev->dev, "rxqlen %d --> %d",
				       temp, dev->rxq.qlen);
...
void axusbnet_disconnect(struct usb_interface *intf)
{
	...
	if (netif_msg_probe(dev))
		dev_info(&dev->udev->dev, "unregister '%s' usb-%s-%s, %s",
...
axusbnet_probe(struct usb_interface *udev, const struct usb_device_id *prod)
{
	...
	if (status < 0) {
		dev_err(&dev->udev->dev, "Binding device failed: %d", status);
		goto out1;
	}
	...
	if (status) {
		dev_err(&dev->udev->dev, "net device registration failed: %d", status);
		goto out3;
	}

	if (netif_msg_probe(dev))
		dev_info(&dev->udev->dev, "register '%s' at usb-%s-%s, %s, %pM",
