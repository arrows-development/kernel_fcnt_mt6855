...
#define AW99703_DRIVER_VERSION "v1.6.0"
#define AW_I2C_RETRIES		5
#define AW_I2C_RETRY_DELAY	2

struct aw99703_data *g_aw99703_data;
static int g_bl_id = 0;
#define BL_AW99703_ID 0x03
#define BL_KTD3136_ID 0x18
static char bl_level[] = {0x07, 0xff};
...
static int aw99703_backlight_init(struct aw99703_data *drvdata)
{
	...
	} else if (g_bl_id == BL_KTD3136_ID) {
		aw99703_i2c_write(drvdata->client, KTD3136_REG_CTR, 0x46);
		aw99703_i2c_write(drvdata->client, KTD3136_REG_LEDLSB, 0x07);
		aw99703_i2c_write(drvdata->client, KTD3136_REG_LEDMSB, 0xFF);
		aw99703_i2c_write(drvdata->client, KTD3136_REG_PWM_EN, 0x9F);
		aw99703_i2c_write(drvdata->client, KTD3136_REG_DIMING, 0x13);
		aw99703_i2c_write(drvdata->client, KTD3136_REG_WORK, 0x99);
	}
...
static int aw99703_read_chipid(struct aw99703_data *drvdata)
{
	...
	while (cnt < AW_READ_CHIPID_RETRIES) {
		...
		case BL_AW99703_ID:
			...
			return 0;
		case BL_KTD3136_ID:
			pr_info("%s ktd3136 detected (0x%x)\n", __func__);
			g_bl_id = value;
			return 0;
		default:
...
void aw99703_bl_en(unsigned int level)
{
	pr_info("%s start.\n", __func__);

	bl_level[0] = (level & 0x0007);
	bl_level[1] = (level >> 3) & 0xff;

	if (level > 0) {
		if (!g_aw99703_data->enable) {
			g_aw99703_data->hwen_gpio = devm_gpiod_get(g_aw99703_data->dev, "aw99703_hwen", GPIOD_OUT_HIGH);
			gpiod_set_value(g_aw99703_data->hwen_gpio, 1);
			devm_gpiod_put(g_aw99703_data->dev, g_aw99703_data->hwen_gpio);
			mdelay(1);
			aw99703_backlight_init(g_aw99703_data);
			if (g_bl_id == BL_AW99703_ID) {
				aw99703_backlight_enable(g_aw99703_data);
			}
			g_aw99703_data->enable = true;
		}
		if (g_bl_id == BL_AW99703_ID) {
			aw99703_i2c_write(g_aw99703_data->client, AW99703_REG_LEDLSB,
				bl_level[0]);
			aw99703_i2c_write(g_aw99703_data->client, AW99703_REG_LEDMSB,
				bl_level[1]);
		} else {
			if (g_bl_id == BL_KTD3136_ID) {
				aw99703_i2c_write(g_aw99703_data->client, KTD3136_REG_LEDLSB,
					bl_level[0]);
				aw99703_i2c_write(g_aw99703_data->client, KTD3136_REG_LEDMSB,
					bl_level[1]);
			}
		}
	} else {
		if (g_aw99703_data->enable) {
			if (g_bl_id == BL_AW99703_ID) {
				aw99703_set_work_mode(g_aw99703_data, 0);
			}

			g_aw99703_data->hwen_gpio = devm_gpiod_get(g_aw99703_data->dev, "aw99703_hwen", GPIOD_OUT_HIGH);
			gpiod_set_value(g_aw99703_data->hwen_gpio, 0);
			devm_gpiod_put(g_aw99703_data->dev, g_aw99703_data->hwen_gpio);
			g_aw99703_data->enable = false;
		}
	}
	...
	struct aw99703_data *drvdata)
{
	...
	drvdata->hwen_gpio = devm_gpiod_get(dev, "aw99703_hwen", GPIOD_OUT_HIGH);
	if (IS_ERR(drvdata->hwen_gpio)) {
		dev_info(dev, "cannot get drvdata->hwen_gpio %ld\n",
			 PTR_ERR(drvdata->hwen_gpio));
		return;
	}
	devm_gpiod_put(dev, drvdata->hwen_gpio);
	drvdata->dev = dev;
