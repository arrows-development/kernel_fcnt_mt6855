...
#define HAPTIC_NV_DRIVER_VERSION	"v1.13.0"
...
static int ram_config(struct aw_haptic *aw_haptic, int duration)
{
	...
	if ((duration > 0) && (duration < aw_haptic->info.duration_time[0])) {
		...
		aw_haptic->activate_mode = AW_RAM_MODE;
			...
			(duration < aw_haptic->info.duration_time[3])) {
		wavseq =4;
		aw_haptic->activate_mode = AW_RAM_MODE;
        } else if (duration >= aw_haptic->info.duration_time[3]) {
		if (aw_haptic->status == 0) {
			wavseq = 5;
		} else if (aw_haptic->status == 1) {
			wavseq = 6;

		}
                 wavloop = 15;
		 aw_info("wavseq=%d",wavseq);
...
static ssize_t cam_status_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);
    return snprintf(buf, PAGE_SIZE, "cam_status = 0x%02X\n", aw_haptic->status);
}

static ssize_t cam_status_store(struct device *dev, struct device_attribute *attr,
				       const char *buf, size_t count)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);
...
static DEVICE_ATTR_RW(cam_status);
#ifdef AW_DOUBLE
