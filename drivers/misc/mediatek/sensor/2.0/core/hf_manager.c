...
#define SENSOR_TYPE_GSENSOR    1
#define SENSOR_TYPE_MSENSOR    2
#define SENSOR_TYPE_GYRO    4
#define SENSOR_TYPE_ALS    5
//#define SENSOR_TYPE_PS     8
static char sensor_hardware_info[4] = {0};
...
static struct task_struct *send_id_task;
int als_type = -1;

extern int get_jdtp_project_id(void);
extern int get_gctp_project_id(void);
		...
		uint8_t sensor_type)
{
	...
	list_for_each_entry(manager, &core->manager_list, list) {
		...
		if (!device || !device->support_list)
			continue;
		for (i = 0; i < device->support_size; ++i) {
			if ((device->support_list[i].sensor_type == SENSOR_TYPE_GSENSOR) && (sensor_hardware_info[0] == 0)) {
				if (0 == strcmp(device->support_list[i].name, "lsm6dso_acc")) {
					hq_regiser_hw_info(HWID_GSENSOR, "Vendor:st IC:lsm6dso");
					pr_err("fcnt_sensor_hardware_info Vendor:st IC:lsm6dso\n");
				} else if (0 == strcmp(device->support_list[i].name,"icm4n607_acc")) {
					hq_regiser_hw_info(HWID_GSENSOR, "Vendor:iven_sense IC:icm4n607");
					pr_err("fcnt_sensor_hardware_info Vendor:iven_sense IC:icm4n607\n");
				}
				sensor_hardware_info[0] = 1;
			}

			if ((device->support_list[i].sensor_type == SENSOR_TYPE_GYRO) && (sensor_hardware_info[1] == 0)) {
				if (0 == strcmp(device->support_list[i].name, "lsm6dso_gyro")) {
					hq_regiser_hw_info(HWID_GYRO, "Vendor:st IC:lsm6dso");
					pr_err("fcnt_sensor_hardware_info Vendor:st IC:lsm6dso\n");
				} else if (0 == strcmp(device->support_list[i].name, "icm4n607_gyro")) {
					hq_regiser_hw_info(HWID_GYRO, "Vendor:iven_sense IC:icm4n607");
					pr_err("fcnt_sensor_hardware_info Vendor:iven_sense IC:icm4n607\n");
				}
				sensor_hardware_info[1] = 1;
			}

			if ((device->support_list[i].sensor_type == SENSOR_TYPE_ALS) && (sensor_hardware_info[2] == 0)) {
				if (0 == strcmp(device->support_list[i].name, "hx32062se_als")) {
					hq_regiser_hw_info(HWID_ALSPS, "Vendor:tyhx IC:hx32062se");
					als_type = HX32062SE_IC;
					pr_err("fcnt_sensor_hardware_info Vendor:tyhx IC:hx32062se\n");
				} else if (0 == strcmp(device->support_list[i].name, "stk3a5x_als")) {
					hq_regiser_hw_info(HWID_ALSPS, "Vendor:sensortek IC:stk3a5x");
					als_type = STK3A5X_IC;
					pr_err("fcnt_sensor_hardware_info Vendor:sensortek IC:stk3a5x\n");
				}
				sensor_hardware_info[2] = 1;
			}

			if ((device->support_list[i].sensor_type == SENSOR_TYPE_MSENSOR) && (sensor_hardware_info[3] == 0)) {
				if (0 == strcmp(device->support_list[i].name, "qmc6308")) {
					hq_regiser_hw_info(HWID_MSENSOR, "Vendor:qst IC:qmc6308");
					pr_err("fcnt_sensor_hardware_info Vendor:qst IC:qmc6308\n");
				} else if (0 == strcmp(device->support_list[i].name, "mmc5603")) {
					hq_regiser_hw_info(HWID_MSENSOR, "Vendor:memsic IC:mmc5603");
					pr_err("fcnt_sensor_hardware_info Vendor:memsic IC:mmc5603\n");
				}
				sensor_hardware_info[3] = 1;
			}
...
static ssize_t pedometer_cust(char *buf, int sensor_type, int action)
{
	ssize_t ret = 0;
	struct hf_client *client = NULL;
	...
	param = simple_strtol(buf, NULL, 16);

	cmd.data[0] = (param >> 24) & 0xFF;
	cmd.data[1] = (param >> 16) & 0xFF;
	cmd.data[2] = (param >> 8) & 0xFF;
	cmd.data[3] = param & 0xFF;
	pr_err("data[0]=%d, data[1]=%d, data[2]=%d, data[3]=%d\n", cmd.data[0], cmd.data[1], cmd.data[2], cmd.data[3]);
...
static ssize_t pedometer_param_show(struct class *dev,
				  struct class_attribute *attr, char *buf)
{
	struct hf_manager *manager = container_of(dev, struct hf_manager,
						     oem_sensor_class);

	return sprintf(buf, "pedometer_param %s\n", manager->pedometer_param);
}

static ssize_t pedometer_param_store(struct class *dev,
				   ...
				   const char *buf, size_t size)
{
	...
	strcpy(manager->pedometer_param, buf);
	pedometer_cust(manager->pedometer_param, SENSOR_TYPE_FCNT_PEDOMETER, CUST_CMD_PEDO);

	return size;
}

static CLASS_ATTR_RW(pedometer_param);
static struct attribute *oem_sensor_class_attrs[] = {
	&class_attr_pedometer_param.attr,
	NULL,
};
ATTRIBUTE_GROUPS(oem_sensor_class);


int send_id_to_sensor(void)
{
	int ret = 0;
	struct hf_client *client = NULL;
	struct custom_cmd cmd;
	uint8_t sensor_type = SENSOR_TYPE_LIGHT;
	...
	memset(&cmd, 0, sizeof(cmd));
	cmd.command = SEND_PROJECT_ID;
	cmd.tx_len = 16;
	cmd.rx_len = 0;

	if(get_jdtp_project_id() != -1){
		cmd.data[0] = get_jdtp_project_id();
	} else if(get_gctp_project_id() != -1) {
	  	cmd.data[0] = get_gctp_project_id();
	}else{
	...
	ret = hf_client_custom_cmd(client,sensor_type,&cmd);
	if(ret < 0){
		pr_err("send project id fail!\n");
		goto out;
	}

	pr_err("send id success!\n");
	return ret;
out:
	hf_client_destroy(client);
	return -EINVAL;
...
static int send_id_thread(void *unused)
{
	while(1)
 	{
		int ret = 0;
          	pr_err("als type = %d\n",als_type);
		if(als_type == HX32062SE_IC){
			ret = send_id_to_sensor();
			if(!ret){
				pr_err("send id success!\n");
				return 0;
			}
		} else if(als_type == STK3A5X_IC){
			pr_err("sensor ic is stk3a5x!\n");
			return 0;
		} else {
			pr_err("not find sensor!,als_type = %d\n",als_type);
		}
		msleep(1000);
	}
	return 0;
...
static int __init hf_manager_init(void)
{
	...
	manager->oem_sensor_class.name = "oem_sensor";
	manager->oem_sensor_class.class_groups = oem_sensor_class_groups;
	ret = class_register(&manager->oem_sensor_class);
	if (ret < 0) {
		pr_err("Failed to create oem_sensor_class ret=%d\n", ret);
		return -1;
	}
	...
	send_id_task = kthread_run(send_id_thread,
                        0, "send_id");
        if (IS_ERR(send_id_task)) {
                pr_err("Failed to create kthread\n");
                ret = PTR_ERR(send_id_task);
                goto err_device;
        }
