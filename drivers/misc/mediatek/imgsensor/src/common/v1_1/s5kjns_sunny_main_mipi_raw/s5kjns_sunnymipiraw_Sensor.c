...
#include "s5kjns_sunnymipiraw_Sensor.h"
#include "s5kjns_sunny_sensor_setting.h"
...
#endif
static kal_uint16 s5kjns_sunny_multi_write_cmos_sensor(
...
static void check_output_stream_off(void)
{
	...
	for (read_count = 0; read_count <= 4; read_count++) {
		read_register0005_value = read_cmos_sensor_byte(0x0005);
...
//		write_cmos_sensor(0x0E12, imgsensor.gain); //aGain 1st frame
		...
		case SHUTTER_32:
			...
			break;
		default:
			LOG_INF("cxc shutter > 1 = %d\n", ((unsigned long)shutter) * 0x05C8 / SHUTTER_1 );
			write_cmos_sensor(0x0E14, ((unsigned long)shutter) * 0x05C8 / SHUTTER_1 ); //2nd frame
			break;
		}
...
static void sensor_init(void)
{
    ...
    s5kjns_sunny_multi_write_cmos_sensor(
		addr_data_pair_init_s5kjns_sunny,
		sizeof(addr_data_pair_init_s5kjns_sunny) / sizeof(kal_uint16));
}
...
static void preview_setting(void)
{
	...
	s5kjns_sunny_multi_write_cmos_sensor(
		addr_data_pair_preview_s5kjns_sunny,
		sizeof(addr_data_pair_preview_s5kjns_sunny) / sizeof(kal_uint16));
}
...
static void capture_setting(kal_uint16 currefps)
{
	LOG_INF("start\n");
	s5kjns_sunny_multi_write_cmos_sensor(
		addr_data_pair_capture_s5kjns_sunny,
		sizeof(addr_data_pair_capture_s5kjns_sunny) / sizeof(kal_uint16));
}
...
static void normal_video_setting(kal_uint16 currefps)  // 30fps
{
	...
	s5kjns_sunny_multi_write_cmos_sensor(
		addr_data_pair_normal_video_s5kjns_sunny,
		sizeof(addr_data_pair_normal_video_s5kjns_sunny) / sizeof(kal_uint16));
}
...
static void hs_video_setting(void) /// 120fps
{
	...
	s5kjns_sunny_multi_write_cmos_sensor(
		addr_data_pair_hs_video_s5kjns_sunny,
		sizeof(addr_data_pair_hs_video_s5kjns_sunny) / sizeof(kal_uint16));
}
...
static void slim_video_setting(void) //60fps
{
	LOG_INF("E\n");
	s5kjns_sunny_multi_write_cmos_sensor(
		addr_data_pair_slim_video_s5kjns_sunny,
		sizeof(addr_data_pair_slim_video_s5kjns_sunny) / sizeof(kal_uint16));
}
...
static void custom1_setting(void)
{
	LOG_INF("E\n");
	s5kjns_sunny_multi_write_cmos_sensor(
		addr_data_pair_custom1_s5kjns_sunny,
		sizeof(addr_data_pair_custom1_s5kjns_sunny) / sizeof(kal_uint16));
}

static kal_uint32 return_sensor_id(void)
{
	// return ((read_cmos_sensor_byte(0x0001) << 8) | read_cmos_sensor_byte(0x0000));
		...
		do {
			*sensor_id = return_sensor_id();
			if ( (*sensor_id) == (imgsensor_info.sensor_id) ) {
				...
				ReadModuleId = read_module_id(0x000D);
				if (ReadModuleId == 0x53) {
					LOG_INF("sunny read_module_id sucess: %x\n", ReadModuleId);
					*sensor_id = S5KJNS_SUNNY_MAIN_SENSOR_ID;
					return ERROR_NONE;
				} else{
					LOG_INF("read_module_id failed: %x\n",
					imgsensor.i2c_write_id, *sensor_id, ReadModuleId);
					*sensor_id = 0xFFFFFFFF;
					return ERROR_SENSOR_CONNECT_FAIL;
				}
			} else {
				LOG_INF("i2c write id: 0x%x, read sensor id:0x%x failed\n", imgsensor.i2c_write_id, *sensor_id);
			}
		    retry--;
		} while (retry > 0);
		i++;
...
kal_uint16 addr_data_pair_set_test_pattern_s5kjns_sunny[] = {
	0x0200,0x0002,
	0x0202,0x0002,
	0x0204,0x0020,
	0x020E,0x0100,
