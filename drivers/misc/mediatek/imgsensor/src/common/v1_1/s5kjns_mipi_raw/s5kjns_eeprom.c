...
#define S5KJNS_EEPROM_READ_ID    0xA1
#define S5KJNS_EEPROM_WRITE_ID   0xA0
#define S5KJNS_I2C_SPEED         100
//#define s5kjns_MAX_OFFSET		0xFFFF
...
static bool selective_read_eeprom(kal_uint16 addr, BYTE *data)
{
	...
	if (iReadRegI2C(pu_send_cmd,
		...
		(u8 *) data,
		1,
		S5KJNS_EEPROM_READ_ID) < 0)
		return false;
	return true;
}
#endif
static bool _read_s5kjns_eeprom(kal_uint16 addr, BYTE *data, int size)
{
	int i = 0;
	int offset = addr;
	int ret;
...
bool s5kjns_read_4cell_otp(char *data, int size, int manual)
{
	int addr = FCELL_START_ADDR;

	pr_debug("read s5kjns cell, size = %d\n", size);
	if (manual || !get_done_4cell || last_size_4cell != size) {
		if (!_read_s5kjns_eeprom(addr, data, size)) {
			get_done_4cell = 0;
			last_size_4cell = 0;
			return false;
		}
...
bool s5kjns_read_otp_ggc(BYTE *data)
{

	int addr = GGC_START_ADDR;
	int size = 346;

	pr_debug("read s5kjns ggc, size = %d\n", size);
