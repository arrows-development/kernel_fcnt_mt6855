...
#include "s5k4h7_mipi_raw_Sensor.h"
#include "s5k4h7_otp.h"
...
int GetCRC(const BYTE *data, const int len)
{
	int crc[16] = { 0 };
	...
	for (i = 0; i < len; i++)
	{
		...
		for (j = 7; j >= 0; j--)
		{
			do_invert = hexonly &(int) (power(2, j));
			do_invert = do_invert / power(2, j);
	...
	for (i = 0; i < 16; i++)
	{
		mycrc16x = mycrc16x + (crc[i] * power(2, i));
	}
	return mycrc16x;
}
 ...
 * ******************************************************/
bool s5k4h7_apply_otp_lsc(void)
{
...
void s5k4h7_get_module_info(unsigned char* ucMIFAWB_data)
{
	...
	for(i= 4; i < 12; i++)
	{
		LOG_INF("%02x",ucMIFAWB_data[i]);
	}

	LOG_INF("VCM ID = 0x%02x",ucMIFAWB_data[12]);
	LOG_INF("Lens ID = 0x%02x",ucMIFAWB_data[13]);
	LOG_INF("Manufacturer ID[0] = 0x%02x",ucMIFAWB_data[14]);
	LOG_INF("Manufacturer ID[1] = 0x%02x",ucMIFAWB_data[15]);
	LOG_INF("Year = %d",ucMIFAWB_data[19]+2000);
	LOG_INF("Month = %d",ucMIFAWB_data[20]);
	LOG_INF("day = %d",ucMIFAWB_data[21]);

	// LOG_INF("Mirror/Flip Status = 0x%02x",ucMIFAWB_data[11]);
	// LOG_INF("IR filter ID = 0x%02x",ucMIFAWB_data[12]);
}

bool s5k4h7_mifawb_datacheck(unsigned char page)
{
	kal_uint8 ucMIFAWB_data[60] = {0x00};
	...
	if (Write_success() ==1)
	{
		for( i=0; i < 8; i++)
		{
			ucMIFAWB_data[i] = read_cmos_sensor_8(0x0A3C + i);
			LOG_INF("ucMIFAWB_data[%d] = 0x%02x",i,read_cmos_sensor_8(0x0A3C + i));
		}
		page+=1;
	}
	...
	if (Write_success() ==1)
	{
		for( i=0; i < 51; i++)
		{
			ucMIFAWB_data[8+i] = read_cmos_sensor_8(0x0A04 + i);
			LOG_INF("ucMIFAWB_data[%d] = 0x%02x",8+i,read_cmos_sensor_8(0x0A04 + i));
			uiChecksum_cal = GetCRC(ucMIFAWB_data, 57);
			// LOG_INF("MIF&AWB calculate checksum = 0x%04x\n",uiChecksum_cal);
		}
	}

	gloden_R=(ucMIFAWB_data[2])|(ucMIFAWB_data[1]<<8);
	gloden_Gr=(ucMIFAWB_data[4])|(ucMIFAWB_data[3]<<8);
	gloden_Gb=(ucMIFAWB_data[6])|(ucMIFAWB_data[5]<<8);
	gloden_B=(ucMIFAWB_data[8])|(ucMIFAWB_data[7]<<8);

	LOG_INF("gloden_R = 0x%04x,gloden_Gr= 0x%04x,gloden_Gb= 0x%04x,gloden_B= 0x%04x\n",gloden_R,gloden_Gr,gloden_Gb,gloden_B);

	uiChecksum_read = (ucMIFAWB_data[58])|(ucMIFAWB_data[57]<<8);;
...
bool s5k4h7_lsc_datacheck(unsigned int group)
{
	...
	{
		// LOG_INF("LSC data[%d] = 0x%02x\n",i,ucLSCData[i]);
		uiChecksum_cal = GetCRC(ucLSCData, 1869);
	...
	{
		LOG_INF("s5k4h7 LSC  checksum success!");
		return 1;
	}
}
...
bool s5k4h7_get_module_lsc(void)
{
	...
	unsigned char page[3] = {36,65,94};  //page 36
	unsigned int group = 0,i=0;

	for (i = 0; i < 3; i++)
	{
	write_cmos_sensor_8(0x0A02, page[i]);
	write_cmos_sensor_8(0x0A00, 0x01);
	mDELAY(10);
	}
...
bool s5k4h7_otp_read(void)
{
	...
	for(group = 0;group < S5K4H7_MIFAWB_GROUP_CNT;group++)
	{
		...
		case 0x40:
			...
			nRet = s5k4h7_mifawb_datacheck(page[group]);
			nRet = s5k4h7_lsc_datacheck(group);
			return nRet;
		case 0xC0:
...
bool s5k4h7_otp_update(void)
{
	...
	for(group = 0;group < S5K4H7_MIFAWB_GROUP_CNT;group++)
	{
		...
		case 0x40:
			LOG_INF("group %d MIF&AWB is valied!\n",group + 1);
			nRet = s5k4h7_update_awb(page[group]);
			nRet = s5k4h7_apply_otp_lsc();
			return nRet;
		case 0xC0:
