...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if (sensorID == 0 || sensorID == 0xFFFFFFFF) {
		...
		err = ERROR_NONE;
		if (sensorID == S5KJNS_SENSOR_ID) {
			hq_regiser_hw_info(HWID_MAIN_CAM, "vendor:qtech,IC:s5kjns");
		} else if (sensorID == S5KJNS_SENSOR_ID - 1) {
			hq_regiser_hw_info(HWID_MAIN_CAM, "vendor:sunny,IC:s5kjns");
		} else if (sensorID == S5K4H7_SENSOR_ID) {
			hq_regiser_hw_info(HWID_SUB_CAM, "vendor:sunwin,IC:s5k4h7");
		} else if (sensorID == S5K4H7_SENSOR_ID - 1) {
			hq_regiser_hw_info(HWID_SUB_CAM, "vendor:sunny,IC:s5k4h7");
		}else if (sensorID == SC202A_SENSOR_ID) {
			hq_regiser_hw_info(HWID_MAIN_CAM_1, "vendor:sunny&sunwin,IC:sc202a");
		}
	}
