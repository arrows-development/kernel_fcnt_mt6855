...
#include "sc202a_mipi_raw_Sensor.h"
...
static char otpData[DATASIZE]={0};
static kal_int8 sc202cs_read_general_info(int addr, kal_uint16 addr_len, kal_uint8 *result)
{
    ...
    write_cmos_sensor(0x440b, 0x12);
  	...
  	for (i = 0; i < addr_len; i++) {
        ...
        LOG_INF("wzy read result[%d] is %x", i, result[i]);
  	}
  	return 0;
}
...
unsigned int sc202cs_read_otp(void)
{
    int ret = 0;
    kal_uint8 i;
    ret = sc202cs_read_general_info(DATA_ADDR, DATASIZE, otpData);
    if (ret != 0) {
		LOG_INF("wzy read fuseid failed\n");
		return -1;
	}

    for(i = 0; i < ARRAY_SIZE(otpData); i++) {
      LOG_INF("wzy read otp data1 0x%x", otpData[i]);
    }
    return ret;
}
...
{
	int index = 0;
	LOG_INF(" wzy addr:0x%x ,size:%d",addr, size);

    for(index=0;index<DATASIZE ;index++){
		data[index] = otpData[index];
        //LOG_INF("data[%d] = %x", index, data[index]);
	}
    data[DATASIZE-2] = data[DATASIZE-1]; //copy checksum2 from data[DATASIZE-1] to data[DATASIZE-2]
    //set sensorId as vendorId
    data[DATASIZE] = read_cmos_sensor(0x3108); //0x52
    data[DATASIZE+1] = read_cmos_sensor(0x3107); //0xeb
    //LOG_INF("DATASIZE[%d] = %x:%x", index, data[DATASIZE], data[DATASIZE+1]);

  	return size;
}
EXPORT_SYMBOL(sc202cs_read_otp_info);
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
		    pr_err("wzy return_sensor_id:0x%x\n",*sensor_id);
            if (*sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                sc202cs_read_otp();
                return ERROR_NONE;
            }
            LOG_INF("wzy Read sensor id fail, return_sensor_id: 0x%x\n",*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
	...
	kal_uint8 i = 0;
	 ...
	 while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		 ...
		 do {
			 sensor_id = return_sensor_id();
			 if (sensor_id == imgsensor_info.sensor_id) {				 
				 LOG_INF("wzy i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);   
				 break;
			 }	 
			 LOG_INF("wzy Read sensor id fail, return_sensor_id: 0x%x\n",sensor_id);
			 retry--;
...
UINT32 SC202A_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT  **pfFunc)
{
