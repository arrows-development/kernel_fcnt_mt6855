...
static const int aw36518_current[AW36518_LEVEL_NUM] = {
	25, 50, 85, 100, 125, 150, 175, 200, 225, 250,
...
static int __maybe_unused aw36518_pinctrl_set(struct aw36518_flash *flash, int pin, int state)
{
	int ret = 0;
...
static int aw36518_check_ic_version(struct aw36518_flash *flash)
{
	...
	pr_info("[%s]: FLASHLIGHT_DEVICE_ID = 0x%x ret=%d\n", __func__, reg_val, rval);
	rval = regmap_read(flash->regmap, AW36518_REG_VERSION_ID, &reg_val);
	pr_info("[%s]: FLASHLIGHT_VERSION_ID = 0x%x ret=%d\n", __func__, reg_val, rval);

	if (rval < 0) {
		/* addr not available, recognized as aw36518 */
		pr_info("[%s:%d]: FLASHLIGHT_IC_VERSION = FLASHLIGHT_IC_AW36518\n", __func__, __LINE__);
		flash->ic_version = FLASHLIGHT_IC_AW36518;
	} else if (reg_val & 0x04) {
		pr_info("[%s:%d]: FLASHLIGHT_IC_VERSION = FLASHLIGHT_IC_AW36518A\n", __func__, __LINE__);
...
static int aw36518_mode_ctrl(struct aw36518_flash *flash)
{
	...
	if (FLASHLIGHT_IC_AW36518A == flash->ic_version) {
		...
		rval = regmap_update_bits(flash->regmap, AW36518_REG_ADJUST, 0xff, 0x0C);
		if (rval < 0) return rval;
	}
			      ...
			      enum aw36518_led_id led_no, bool on)
{
	...
	if (led_no == AW36518_LED0) {
		...
		aw36518_pinctrl_set(flash, AW36518_PINCTRL_PIN_HWEN, AW36518_PINCTRL_PINSTATE_LOW);
		}
	}
	return rval;
}
				 ...
				 enum aw36518_led_id led_no, unsigned int brt)
{
	...
	if (led_no == AW36518_LED0)
		rval = regmap_update_bits(flash->regmap, AW36518_REG_LED_TORCH_BRIGHTNESS, 0xff, br_bits);
	return rval;
}
				 ...
				 enum aw36518_led_id led_no, unsigned int brt)
{
	...
	if (led_no == AW36518_LED0)
		rval = regmap_update_bits(flash->regmap, AW36518_REG_LED_FLASH_BRIGHTNESS, 0xff, br_bits);
	return rval;
}
				...
				unsigned int tout)
{
	...
	rval = regmap_update_bits(flash->regmap, AW36518_REG_TIMING_CONF, 0x0f, tout_bits);
	return rval;
}
...
static void aw36518_soft_reset(struct aw36518_flash *flash)
{
	regmap_update_bits(flash->regmap, AW36518_REG_BOOST_CONF, AW36518_BOOST_RESET, AW36518_BOOST_RESET);
	msleep(5);
}
...
static int aw36518_set_ctrl(struct v4l2_ctrl *ctrl, enum aw36518_led_id led_no)
{
	...
	case V4L2_CID_FLASH_LED_MODE:
		...
		else if (flash->led_mode == V4L2_FLASH_LED_MODE_NONE)
...
static int aw36518_init(struct aw36518_flash *flash)
{
	...
	aw36518_soft_reset(flash);

	/* set timeout */
	rval = aw36518_flash_tout_ctrl(flash, AW36518_FLASH_TOUT_DEFAULT);
	if (rval < 0) {
		pr_info("aw36518_flash_tout_ctrl failed: %d", rval);
		return rval;
	}
	...
	rval = aw36518_mode_ctrl(flash);
	if (rval < 0) {
		pr_info("aw36518_mode_ctrl failed: %d", rval);
		return rval;
	}

	rval = regmap_update_bits(flash->regmap, AW36518_REG_LED_TORCH_BRIGHTNESS, 0xff, 0x00);
	if (rval < 0) {
		pr_info("regmap_i2c failed: %d", rval);
		return rval;
	}
	rval = regmap_update_bits(flash->regmap, AW36518_REG_LED_FLASH_BRIGHTNESS, 0xff, 0x00);
	if (rval < 0) {
		pr_info("regmap_i2c failed: %d", rval);
		return rval;
	}
...
// static struct hrtimer aw36518_timer_ch2;
...
{
	pr_info("aw36518_flash_open\n");
	return 0;
}

static int aw36518_flash_release(void)
{
	pr_info("aw36518_flash_release\n");
	return 0;
}
...
static int aw36518_operate(int channel, int enable)
{
	...
	if (enable && (aw36518_flash_data->level >= 0)) {
		...
		if (aw36518_timeout_ms[channel]) {
			...
			hrtimer_start(&aw36518_timer, ktime, HRTIMER_MODE_REL);
		}
...
{
	pr_info("aw36518_strobe_store start (%d, %d, %d), (%d, %d)\n",
	...
	pr_info("aw36518_strobe_store end");
	return 0;
}
...
static const struct of_device_id aw36518_of_table[] = {
	{ .compatible = "mediatek,flashlight-aw36518" },
	{ },
};
...
MODULE_DESCRIPTION("Awinic AW36518/AW36518A LED flash driver");
MODULE_LICENSE("GPL");
